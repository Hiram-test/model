    ...
    with samples_path.open("x", encoding="utf-8", newline="\n") as samples_handle:  # 排他创建监控流水，本文件存在即充当唯一监控认领。
        while True:  # 持续采样直到本 job 进程树稳定退出或完成硬停处置。
            ...
            if related_processes:  # 本轮仍检测到本 job 进程时不能判定求解完成。
                ...
                empty_process_samples = empty_process_samples + 1 if normal_exit_evidence else 0  # 无论是否已提交 ABT，都必须同时证明进程树为空、文件连续稳定且 lock 已移除才累计退出确认。
                previous_terminal_file_state = current_terminal_file_state  # 保存当前三文件大小供下一空进程样本比较。
                if sample_started - post_exit_since >= POST_EXIT_STABILITY_TIMEOUT_SECONDS and not normal_exit_evidence:  # 任何路径在进程树消失六十秒后仍有锁或文件变化时都不得冒充受控自然退出。
