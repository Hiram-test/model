...
def prepare_run(optional_run_name: str | None, user_override_memory_gate: bool) -> Path:  # 输入可选运行名和内存覆盖事实并返回新 S10 目录。
    ...
    if user_override_memory_gate and disk_ready:  # 用户已明确覆盖内存门禁且磁盘达到32 GiB时，使文本策略与机器状态保持一致。
        launch_text = launch_text.replace("REQUIRED_NEXT_ACTION=S10_PREFLIGHT_MEMORY_AND_INDEPENDENT_AUDIT_REQUIRED", "REQUIRED_NEXT_ACTION=INDEPENDENT_AUDIT_THEN_EXPLICIT_DMP4_LAUNCH")  # 移除已被用户覆盖的内存前置动作，只保留独立审计与显式启动要求。
        launch_text = launch_text.replace("RESOURCE_POLICY=DO_NOT_RUN_UNLESS_MEMORY_READY_AND_DISK_READY", "RESOURCE_POLICY=USER_MEMORY_GATE_OVERRIDE_RECORDED_AND_DISK_READY")  # 明确内存采用用户覆盖、磁盘自然通过，避免命令证据自相矛盾。
