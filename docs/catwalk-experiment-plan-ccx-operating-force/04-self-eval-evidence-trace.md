...
| 报结论，错误结论也可以。不报停了/没有/没进账本。你有一个主要的模型，主体，这个东西是一直要往前推的，不能把它改坏了乱改 | 必须报：Umax 巨大、四件套缺失、账本仍写 760c0ee4、Actions 错对象。主模型字节不动 | **成立** | §1–§4 |
...
1. **P0 线性静力（锁）：** ccx exit 0；\(U_{\max}=9.264\times 10^{9}\) mm；eid 1 为 \(15\,687\,915\) N 对 sidecar 中的 MCT \(15\,686\,250\) N。  
2. **\(U_{\max}\) 是可写结论，且很大概率是失败静力。** 不得改写成 符合附件2-3。  
3. **H-ZJG-CCX-OF-001 尚未被检验。** sidecar 把 MCT **INI-EFORCE mean** 当成对照，那是源侧初内力，不是运营力表。  
4. **703.46 不得当结果。** 本岗从哈希 deck 复算出它就是 IC 迹 / \(F/A\)。ANSYS `.db` 抽出 `INISTATE=0`，所谓 703.46280 也不是本岗 POST1。  
5. **自制 deck 不是正路。** 四哈希当场复算仍在；`6712e918` 不在树里，不换主。
...
| 锁 974211b2 三事实 | 是 | `locked-objects.json` |
...
**主模型是 974211b2，还在，没改坏。** 计算结论按锁写：exit 0，Umax \(9.264\times 10^{9}\) mm，eid1 \(15687915\) N vs \(15686250\) N。这是失败静力倾向的结论，不是 符合。  
**703.46 是源侧 IC 迹，已排除出 Results。**  
**四件套和运营力表和猫道 Actions 都是 No-Go，已记账，不是「停了」。**  
**账本 JSON 仍写 760c0ee4，migrate 行未登记；文件已在 #19 分支。**  
本对话交付预注册、MDPI 实验设置草案、claim-trace、本留痕。完整论文总目标未在本对话关闭。
