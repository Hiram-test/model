...
def parse_args() -> argparse.Namespace:  # 无业务输入并返回命令行命名空间；只允许选择 TYPE72 静力切线单变量诊断。
    parser = argparse.ArgumentParser(description="准备恒总荷载位置迁移静力诊断包。")  # 创建仅负责准备新运行且不会启动 MAPDL 的参数解析器。
    parser.add_argument("--exclude-mpc-stress-stiffness", action="store_true", help="仅在静力阶段设置 TYPE72 KEYOPT(5)=1，并保持 0.5% 迁移路径及全部门禁不变。")  # 默认假时复现既有 0.5% 路径；显式为真时执行官方建议的单差异收敛诊断。
...
def transform_main(source_bytes: bytes, old_jobname: str, new_jobname: str, correction_audit: dict[str, Any], exclude_mpc_stress_stiffness: bool = False) -> tuple[bytes, dict[str, Any]]:  # 输入父主控、作业身份、修正审计和 TYPE72 静力切线开关并返回两步位置迁移静力主控。
    ...
    migration_title = "/TITLE,C10 DIRECT MPC CONSTANT-LOAD MIGRATION K5-EXCLUDED STATIC DIAGNOSTIC" if exclude_mpc_stress_stiffness else "/TITLE,C10 DIRECT MPC CONSTANT-TOTAL-LOAD POSITION-MIGRATION STATIC DIAGNOSTIC"  # 仅用标题区分 TYPE72 KEYOPT(5)=1 单差异运行与既有 0.5% 基准运行。
    ...
    if exclude_mpc_stress_stiffness:  # 只有显式单变量诊断才修改 TYPE72 几何应力刚度选项。
    ...
    ls2_autots_new = "! 保留 LS2 自动时间步框架，但 NSUBST 的初始、最大和最小值均为二百，因此实际增量固定为 0.5%。" + newline + "AUTOTS,ON" + newline  # 保持求解器路径格式一致，同时用三项相等的子步合同禁止放大或缩小增量。
    ...
    ls2_nsubst_new = f"! LS2 初始 {LS2_INITIAL_SUBSTEPS}、最多 {LS2_MAX_SUBSTEPS}、最少 {LS2_MIN_SUBSTEPS} 个子步；每个冻结子步只迁移 0.5% 的荷载位置。" + newline + f"NSUBST,{LS2_INITIAL_SUBSTEPS},{LS2_MAX_SUBSTEPS},{LS2_MIN_SUBSTEPS}" + newline  # 建立连续、固定分辨率且不会被放大的位置迁移路径。
    candidate = base.replace_once(candidate, ls2_nsubst_old, ls2_nsubst_new, "LS2 固定二百子步位置迁移")  # 只替换 LS2 的子步合同。
    ...
    require(candidate_lines.count("KEYOPT,72,5,1") == (1 if exclude_mpc_stress_stiffness else 0), "TYPE72 KEYOPT(5)=1 命令数量与诊断模式不一致")  # 确认开关为真时恰一次、为假时完全不存在，关闭隐含多变量路径。
    audit["schema_version"] = 5 if exclude_mpc_stress_stiffness else 4  # 单变量 KEYOPT(5) 诊断升级为第五版，既有 0.5% 路径保持第四版兼容身份。
    audit["change_families"] = [*audit["change_families"], "CONSTANT_TOTAL_LOAD_POSITION_MIGRATION_BETA_1_TO_0", *( ["MPC184_STATIC_GEOMETRIC_STRESS_STIFFNESS_EXCLUSION"] if exclude_mpc_stress_stiffness else [] )]  # 记录共同迁移路径，并仅在开关为真时追加 TYPE72 静力切线变更族。
    ...
    audit["mpc184_keyopt5_static"] = 1 if exclude_mpc_stress_stiffness else 0  # 机器记录两次静力求解实际采用的 TYPE72 KEYOPT(5) 值。
    audit["prestressed_modal_requires_keyopt5_restore_to_zero"] = bool(exclude_mpc_stress_stiffness)  # 开关为真时硬性声明预应力模态前必须恢复几何应力刚度。
...
def main() -> None:  # 读取单变量开关、验证冻结源并生成一个全新且未启动的恒总荷载位置迁移诊断包。
    args = parse_args()  # 解析是否执行 TYPE72 静力几何应力刚度排除诊断，未知参数直接拒绝。
    ...
    previous_migration_run_name = K5_REFERENCE_MIGRATION_RUN_NAME if exclude_mpc_stress_stiffness else PREVIOUS_MIGRATION_RUN_NAME  # KEYOPT(5) 单差异模式引用 0.5% 发散基准，兼容模式继续引用 5% 基准。
    expected_previous_status = "ABORTED_BY_CONTROLLER_AFTER_LS2_FIRST_0_5_PERCENT_MIGRATION_DIVERGED" if exclude_mpc_stress_stiffness else "ABORTED_BY_CONTROLLER_AFTER_LS2_FIRST_5_PERCENT_MIGRATION_DIVERGED"  # 冻结两种模式各自允许继承的唯一前序根状态。
    previous_increment_fraction = 0.005 if exclude_mpc_stress_stiffness else 0.05  # 单差异模式保持 0.5% 不变，兼容模式从 5% 缩小到 0.5%。
    new_increment_fraction = 0.005  # 两种模式的新 LS2 都固定二百个子步，即每步迁移总路径的 0.5%。
    step_refinement_factor = 1 if exclude_mpc_stress_stiffness else 10  # KEYOPT(5) 模式禁止步长变化，兼容模式登记十倍细化。
    single_variable_change = "TYPE72_KEYOPT5_0_TO_1_ONLY" if exclude_mpc_stress_stiffness else "LS2_INCREMENT_5_PERCENT_TO_0_5_PERCENT_ONLY"  # 明确本次与直接前序运行之间唯一获准的输入变化。
    diagnostic_subtype = "CONSTANT_TOTAL_LOAD_POSITION_MIGRATION_WITH_MPC184_STATIC_STRESS_STIFFNESS_EXCLUDED" if exclude_mpc_stress_stiffness else "CONSTANT_TOTAL_LOAD_POSITION_MIGRATION_BETA_1_TO_0"  # 为根状态和清单生成互斥诊断子类型。
    ...
    job_family = "cw_C10mk5" if exclude_mpc_stress_stiffness else "cw_C10m"  # 用短 ASCII 前缀在 MAPDL 文件层区分 KEYOPT(5) 单差异和普通迁移运行。
    ...
    migration_bytes, change_audit = transform_main(parent_main_bytes, old_jobname, new_jobname, correction_audit, exclude_mpc_stress_stiffness)  # 生成 LS1 旧位置平衡、LS2 恒总荷载迁移及可选 TYPE72 单差异静力主控。
    ...
    base.write_json(qa_dir / "previous_migration_reference.json", {"schema_version": 2 if exclude_mpc_stress_stiffness else 1, "status": previous_migration_status["status"], "run_name": previous_migration_run_name, "root_status_sha256": base.sha256_file(previous_migration_dir / "C10_static_status.json"), "runtime_abort_audit_sha256": base.sha256_file(previous_migration_dir / "qa" / "runtime_abort_audit.json"), "ls1_beta_1_converged": True, "previous_first_increment_fraction": previous_increment_fraction, "new_increment_fraction": new_increment_fraction, "step_refinement_factor": step_refinement_factor, "single_variable_change": single_variable_change})  # 冻结直接前序失败证据、相同步长或十倍细化关系及唯一获准变化。
    ...
    status = {"schema_version": 2 if exclude_mpc_stress_stiffness else 1, "run_name": run_name, "jobname": new_jobname, "status": "STATIC_DIAGNOSTIC_PREPARED", "diagnostic_subtype": diagnostic_subtype, "created_at_utc": created_at.isoformat(), "parent_run": base.PARENT_RUN_NAME, "previous_migration_run": previous_migration_run_name, "single_variable_change": single_variable_change, "mpc184_keyopt5_static": 1 if exclude_mpc_stress_stiffness else 0, "prestressed_modal_requires_keyopt5_restore_to_zero": bool(exclude_mpc_stress_stiffness), "micro_validation_run": base.MICRO_RUN_NAME, "mapdl_execution_attempted": False, "mapdl_started": False, "execution_mode": "SMP_SERIAL_NP1_DIAGNOSTIC_ONLY", "resource_gate": "FORMAL_8_GIB_FAILED_DIAGNOSTIC_EXCEPTION_REQUIRES_RUNTIME_RECHECK", "launch_allowed_for_diagnostic": True, "launch_allowed_for_production": False, "full_bridge_static_status": "LOAD_POSITION_MIGRATION_PREPARED_NOT_STARTED", "full_bridge_modal_status": "NOT_RUN", "valid_for_production": False, "next_action": "RUNTIME_RESOURCE_RECHECK_THEN_EXECUTE_SINGLE_VARIABLE_K5_LOAD_MIGRATION_WITH_MONITORING" if exclude_mpc_stress_stiffness else "RUNTIME_RESOURCE_RECHECK_THEN_EXECUTE_LOAD_POSITION_MIGRATION_WITH_MONITORING"}  # 冻结准备完成、唯一数值变量、模态恢复义务和只允许静力诊断的根状态。
    manifest = {"schema_version": 6 if exclude_mpc_stress_stiffness else 5, "run_name": run_name, "jobname": new_jobname, "status": status["status"], "diagnostic_subtype": status["diagnostic_subtype"], "created_at_utc": created_at.isoformat(), "parent_run": base.PARENT_RUN_NAME, "parent_main_sha256": change_audit["source_sha256"], "previous_migration_run": previous_migration_run_name, "previous_migration_reference": "qa/previous_migration_reference.json", "migration_increment_change": {"previous_fraction": previous_increment_fraction, "new_fraction": new_increment_fraction, "refinement_factor": step_refinement_factor}, "single_variable_change": single_variable_change, "mpc184_keyopt5_static": 1 if exclude_mpc_stress_stiffness else 0, "prestressed_modal_requires_keyopt5_restore_to_zero": bool(exclude_mpc_stress_stiffness), "preparer_script": f"input_snapshot/{Path(__file__).name}", "preparer_script_sha256": base.sha256_file(Path(__file__).resolve()), "base_preparer_script_sha256": PREPARER_BASE_SHA256, "micro_validation_run": base.MICRO_RUN_NAME, "micro_validation_status": micro_results["status"], "constraint_topology": "SINGLE_TYPE72_NO_AUX_NO_TYPE73", "expected_topology": parent_manifest["expected_topology"], "main_input": f"solver/{MIGRATION_MAIN_NAME}", "main_input_sha256": change_audit["candidate_sha256"], "load_position_correction_include": f"solver/{CORRECTION_INCLUDE_NAME}", "load_position_correction_include_sha256": correction_audit["include_sha256"], "load_position_migration_audit": "qa/load_position_migration_audit.json", "mapdl_executable": str(base.MAPDL_EXE), "mapdl_executable_sha256": base.sha256_file(base.MAPDL_EXE), "execution_mode": status["execution_mode"], "analysis_scope": "FULL_BRIDGE_STATIC_LS1_OLD_LOAD_POSITION_BALANCE_AND_LS2_CONSTANT_TOTAL_LOAD_POSITION_MIGRATION", "load_path_mode": "BETA_1_OLD_MCT_LOAD_POSITION_TO_BETA_0_SPATIAL_MASS21_AT_CONSTANT_TOTAL_LOAD", "initial_state_load_path": "MCT_INISTATE_PLUS_FULL_GRAVITY_AT_OLD_BALANCED_POSITION_THEN_CONTINUOUS_POSITION_MIGRATION", "initial_state_equilibrium_audit": "PENDING_FULL_STATIC_SOLUTION_AND_INDEPENDENT_BALANCE_CHECK", "constraint_imposition": "DIRECT_ELIMINATION", "penalty_n_per_mm": None, "modal_requested": False, "production_claim_allowed": False, "static_result_expected": True, "runtime_equation_count_change_allowed": False, "runtime_small_zero_negative_pivot_allowed": False, "runtime_ignored_or_reset_cnvtol_allowed": False, "launch_argv": launch_argv}  # 汇总直接前序证据、唯一变量、求解器、拓扑、守恒向量、运行时硬门和禁止生产外推边界。
    ...
    print(json.dumps({"run_dir": str(run_dir), "jobname": new_jobname, "status": status["status"], "diagnostic_subtype": diagnostic_subtype, "mpc184_keyopt5_static": 1 if exclude_mpc_stress_stiffness else 0, "correction_node_count": len(corrections), "rendered_correction_sum_n": correction_audit["rendered_correction_sum_n"]}, ensure_ascii=False))  # 向调用者返回可解析的唯一目录、作业名、单变量模式和守恒摘要。
