...
def main() -> None:  # 无输入参数；验证冻结源并生成一个全新、未启动的恒总荷载位置迁移诊断包。
    ...
    previous_migration_dir = base.RUNS_ROOT / PREVIOUS_MIGRATION_RUN_NAME  # 定位 5% 首步被控制器停止的直接前序迁移运行。
    previous_migration_status = base.load_json(previous_migration_dir / "C10_static_status.json")  # 读取前序根状态供步长缩小因果门禁。
    require(previous_migration_status.get("status") == "ABORTED_BY_CONTROLLER_AFTER_LS2_FIRST_5_PERCENT_MIGRATION_DIVERGED", "前序迁移运行状态不是已冻结的 5% 步长发散")  # 禁止在证据未闭合时擅自缩小步长。
    require(previous_migration_status.get("ls1_beta_1_converged") is True, "前序迁移运行未证明 beta=1 基态收敛")  # 新运行必须继承已证实的物理起点而非另一条假设。
    require(float(previous_migration_status.get("ls2_attempted_migration_fraction", -1.0)) == 0.05, "前序迁移首步不是冻结的 5%")  # 固定十倍细化的比较基准。
    ...
    base.write_json(qa_dir / "previous_migration_reference.json", {"schema_version": 1, "status": previous_migration_status["status"], "run_name": PREVIOUS_MIGRATION_RUN_NAME, "root_status_sha256": base.sha256_file(previous_migration_dir / "C10_static_status.json"), "runtime_abort_audit_sha256": base.sha256_file(previous_migration_dir / "qa" / "runtime_abort_audit.json"), "ls1_beta_1_converged": True, "previous_first_increment_fraction": 0.05, "new_increment_fraction": 0.005, "step_refinement_factor": 10})  # 冻结从 5% 发散到 0.5% 新尝试的直接证据和唯一数值变化。
