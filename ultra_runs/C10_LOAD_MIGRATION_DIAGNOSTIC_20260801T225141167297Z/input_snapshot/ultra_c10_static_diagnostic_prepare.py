...
def parse_args() -> argparse.Namespace:  # 无业务输入；解析命令行并返回是否启用残差定位的命名空间。
    ...
    mode_group = parser.add_mutually_exclusive_group()  # 建立互斥模式组，防止不同物理或数值诊断变量在同一运行中混用。
    ...
    mode_group.add_argument("--exclude-mpc-stress-stiffness", action="store_true", help="按 MPC184 官方建议仅在非线性静力阶段排除 TYPE72 几何应力刚度")  # 布尔开关只改变 TYPE72 的 KEYOPT(5)，并保留父运行的二十子步空间重力路径、直接消元运动学和四项收敛门。
...
def transform_main(source_bytes: bytes, old_jobname: str, new_jobname: str, exclude_mpc_stress_stiffness: bool = False) -> tuple[bytes, dict[str, Any]]:  # 输入父主控字节、新旧作业名和 TYPE72 静力切线开关，返回静力专用主控与变更审计。
    ...
    diagnostic_title = "/TITLE,C10 DIRECT MPC STATIC STRESS-STIFFNESS-EXCLUDED RAMP DIAGNOSTIC" if exclude_mpc_stress_stiffness else "/TITLE,C10 DIRECT MPC STATIC FULL-FORMED-STATE EQUILIBRIUM-PAIR DIAGNOSTIC"  # 根据唯一模式选择官方 KEYOPT(5) 收敛诊断或既有形成态配平诊断标题。
    candidate = replace_once(candidate, "/TITLE,C10 MPC184 CONNECTION PATCH FULL BRIDGE PRESTRESSED MODAL", diagnostic_title, "诊断标题")  # 明确本运行的单一诊断变量且仍只限静力诊断。
    if exclude_mpc_stress_stiffness:  # 只有显式官方收敛诊断才排除刚臂几何应力刚度并保留其他父模型定义。
        prep7_anchor = "! 进入前处理器读取完整装配后的节点和单元身份。" + newline + "/PREP7" + newline  # 冻结全部 include 装配完成后且仍处于前处理器的唯一插入锚点。
        prep7_block = prep7_anchor + "! 按 MPC184 单元文档建议，仅在非线性静力诊断中排除 rigid-beam 几何应力刚度对切线矩阵的贡献。" + newline + "KEYOPT,72,5,1" + newline  # 保留 TYPE72 直接消元刚体运动学，只切换可能造成罕见收敛困难的 KEYOPT(5)。
        candidate = replace_once(candidate, prep7_anchor, prep7_block, "TYPE72 静力几何应力刚度排除")  # 在计数和首个 SOLVE 前唯一设置 TYPE72 KEYOPT(5)=1。
    ...
    if not exclude_mpc_stress_stiffness:  # 既有形成态、残差和 S10 种子诊断继续使用完整恒载单步配对路径。
    ...
    if exclude_mpc_stress_stiffness:  # 官方 KEYOPT(5) 单差异诊断必须逐字保留父运行的斜坡荷载合同。
        require(candidate.count("KEYOPT,72,5,1") == 1, "TYPE72 KEYOPT(5)=1 不是唯一一次")  # 确认静力切线排除命令既未遗漏也未重复。
        require(candidate.count("KBC,0") == 2 and "KBC,1" not in candidate, "父运行 LS1/LS2 斜坡 KBC 合同发生漂移")  # 确认两个荷载步都保留 KBC=0。
        require(candidate.count("NSUBST,20,200,20") == 1 and candidate.count("NSUBST,1,1,1") == 1, "父运行 20 子步加载与单步保持合同发生漂移")  # 确认 LS1 与 LS2 子步控制未被形成态变换改写。
        require(candidate.count("AUTOTS,ON") == 1 and candidate.count("AUTOTS,OFF") == 1, "父运行自动步长合同发生漂移")  # 确认 LS1 允许细分而 LS2 禁止细分。
    else:  # 其他三种诊断继续执行既有完整形成态配平硬门。
    ...
    change_families = ["UNIQUE_DIAGNOSTIC_IDENTITY", "EXPLICIT_NONRELAXING_CONVERGENCE_CRITERIA", "MPC184_STATIC_GEOMETRIC_STRESS_STIFFNESS_EXCLUSION", "PARENT_RAMP_LOAD_PATH_PRESERVED", "STATIC_ONLY_TRUNCATION"] if exclude_mpc_stress_stiffness else ["UNIQUE_DIAGNOSTIC_IDENTITY", "EXPLICIT_NONRELAXING_CONVERGENCE_CRITERIA", "FULL_FORMED_STATE_LOAD_PAIRED_WITH_INISTATE", "STATIC_ONLY_TRUNCATION"]  # 冻结单差异 KEYOPT(5) 诊断或既有形成态诊断的互斥变更族。
    load_path_change_allowed = False if exclude_mpc_stress_stiffness else True  # KEYOPT(5) 诊断禁止改变父荷载路径，形成态诊断则保留已登记的路径变更。
    load_path_physical_basis = "PARENT_C10_RAMP_PATH_PRESERVED_TO_ISOLATE_MPC184_KEYOPT5_CONVERGENCE_EFFECT" if exclude_mpc_stress_stiffness else "MCT_FORMED_STAGE_ALOAD_AND_INITIAL_STATE_BALANCE_REQUIRE_FULL_BALANCING_LOAD_AT_FIRST_POINT"  # 记录荷载路径保持或形成态配平的物理依据。
    load_path_physical_approval = "OFFICIAL_MPC184_CONVERGENCE_DIAGNOSTIC_PENDING_FULL_STATIC_BALANCE_AND_MODAL_REENABLE_CHECK" if exclude_mpc_stress_stiffness else "DIAGNOSTIC_HYPOTHESIS_PENDING_FULL_RECONCILIATION"  # KEYOPT(5) 只获准作为静力诊断且模态前必须恢复几何应力刚度。
    ls1_audit = {"kbc": 0, "autots": True, "nsubst": [20, 200, 20], "time": 1.0, "physical_role": "PARENT_RAMPED_SPATIAL_MASS21_GRAVITY_WITH_MPC184_STRESS_STIFFNESS_EXCLUDED"} if exclude_mpc_stress_stiffness else {"kbc": 1, "autots": False, "nsubst": [1, 1, 1], "time": 1.0, "physical_role": "FULL_PERMANENT_LOAD_PAIRED_WITH_FULL_INISTATE"}  # 冻结两种模式互斥的 LS1 控制合同。
    audit = {"schema_version": 4, "status": "PASSED", "source_sha256": sha256_bytes(source_bytes), "candidate_sha256": sha256_bytes(candidate.encode("utf-8")), "parent_jobname_reference_count": 5, "diagnostic_jobname_reference_count": 3, "change_families": change_families, "load_path_change_allowed": load_path_change_allowed, "load_path_physical_basis": load_path_physical_basis, "load_path_physical_approval": load_path_physical_approval, "mpc184_keyopt5_static": 1 if exclude_mpc_stress_stiffness else 0, "prestressed_modal_requires_keyopt5_restore_to_zero": bool(exclude_mpc_stress_stiffness), "ls1": ls1_audit, "ls2": {"kbc": 0, "autots": False, "nsubst": [1, 1, 1], "time": 1.001, "physical_load_increment": 0.0, "physical_role": "ZERO_INCREMENT_EQUILIBRIUM_HOLD"}, "cnvtol": {"force_relative": float(FORCE_TOLERANCE), "moment_relative": float(MOMENT_TOLERANCE), "displacement_relative": float(DISPLACEMENT_TOLERANCE), "rotation_relative": float(ROTATION_TOLERANCE), "automatic_relaxation_allowed": False, "ignored_command_allowed": False}, "modal_commands_present": False, "production_claim_allowed": False}  # 汇总唯一数值变量、荷载路径、四项不可放宽准则、静力截断范围和模态恢复义务。
...
def main() -> None:  # 无输入参数；验证冻结谱系并生成一个新的静力诊断运行包，无业务返回值。
    ...
    exclude_mpc_stress_stiffness = bool(args.exclude_mpc_stress_stiffness)  # 将 TYPE72 静力几何应力刚度排除开关规范化为输入、清单和说明共用标志。
    ...
    diagnostic_bytes, change_audit = transform_main(parent_main_bytes, old_jobname, new_jobname, exclude_mpc_stress_stiffness)  # 按唯一模式生成形成态配平或 TYPE72 KEYOPT(5) 单差异静力输入，保留四项收敛准则并截断模态。
    ...
    elif exclude_mpc_stress_stiffness:  # 为官方 MPC184 静力收敛建议选择父荷载路径不变的单差异合同。
        diagnostic_subtype = "MPC184_STATIC_GEOMETRIC_STRESS_STIFFNESS_EXCLUDED_RAMP"  # 标识静力阶段唯一改变 TYPE72 KEYOPT(5)=1 的诊断子类型。
        analysis_scope = "FULL_BRIDGE_STATIC_LS1_PARENT_RAMP_LS2_HOLD_AND_STATIC_GATES_WITH_TYPE72_KEYOPT5_1"  # 允许父运行二十子步空间重力、零增量保持和全部静力门执行。
        next_action = "RUNTIME_RESOURCE_RECHECK_THEN_EXECUTE_TYPE72_KEYOPT5_STATIC_DIAGNOSTIC_WITH_MONITORING"  # 冻结启动后的唯一动作。
        full_bridge_static_status = "MPC184_STRESS_STIFFNESS_EXCLUSION_PREPARED_NOT_STARTED"  # 明确官方建议尚未接受全桥求解验证。
        load_path_mode = "PARENT_C10_RAMPED_SPATIAL_MASS21_GRAVITY_UNCHANGED"  # 记录本模式不得修改父 C10 的荷载路径。
        initial_state_load_path = "MCT_INISTATE_PLUS_PARENT_RAMPED_SPATIAL_MASS21_GRAVITY"  # 记录初始状态与空间质量重力沿父路径组合。
        initial_state_audit = "MPC184_KEYOPT5_STATIC_DIAGNOSTIC_PENDING_FULL_STATIC_BALANCE"  # 禁止把官方建议直接外推为已闭合平衡。
        static_result_expected = True  # 该模式允许执行完整静力和门禁，但仍不预先宣称通过。
