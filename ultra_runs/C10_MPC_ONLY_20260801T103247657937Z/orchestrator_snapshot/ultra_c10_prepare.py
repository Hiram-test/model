...
def transform_main(source_bytes: bytes, jobname: str) -> tuple[bytes, dict[str, Any]]:  # 输入最终 S10 主控和新 jobname 并返回可逆 C10 主控及审计。
    ...
    audit = {"schema_version": 1, "status": "PASSED", "source_sha256": sha256_bytes(source_bytes), "candidate_sha256": sha256_bytes(candidate.encode("utf-8")), "canonicalized_sha256": sha256_bytes(canonical_bytes), "canonicalized_equals_source_bytes": True, "identity_changes": ["S10_TO_C10_LABELS", "UNIQUE_JOBNAME", "OUTPUT_PREFIXES"], "topology_changes": {"node_count": [EXPECTED_PARENT_NODES, EXPECTED_CANDIDATE_NODES], "element_count": [EXPECTED_PARENT_ELEMENTS, EXPECTED_CANDIDATE_ELEMENTS], "type72_expected": EXPECTED_LOGICAL_CONNECTIONS, "type73_expected": EXPECTED_UXYZ_CONNECTIONS}, "added_runtime_audits": ["TYPE72_COUNT", "TYPE73_COUNT", "V2_MPC184_E_ELIST"], "unchanged_control_contract": ["DEPENDENCY_ORDER", "LS1_STATIC_FROM_ZERO", "LS2_HOLD", "NLGEOM", "PSTRES", "LANB_80_NO_FREQUENCY_CAP", "MXPAND_ELEMENT_CALC", "NSOL_VECTOR_EXPORT", "VENG_HISTORY_EXPORT"]}  # 汇总主控可逆白名单。
...
def build_hash_rows(parent_snapshot: Path, input_snapshot: Path, solver_dir: Path, candidate_include_hash: str, candidate_main_hash: str) -> list[dict[str, Any]]:  # 输入三目录和候选哈希并返回双副本输入审计行。
    ...
    for order, name in enumerate(DEPENDENCY_NAMES, start=1):  # 按 /INPUT 顺序写十一项依赖。
        ...
        rows.append({"order": order, "name": name, "change_class": "CONTROLLED_CONNECTION_REPLACEMENT" if controlled else "BYTE_INVARIANT", "parent_sha256": parent_hash, "input_snapshot_sha256": snapshot_hash, "solver_sha256": solver_hash, "all_expected_hashes_match": True})  # 保存有序依赖哈希证据。
...
def prepare(parent_run: str, run_name: str, penalty_n_per_mm: float) -> Path:  # 输入父身份、目标身份和基准罚因子并返回生成运行目录。
    ...
    pre_solve_issues = {"schema_version": 1, "status": "VALIDATION_GATES_PENDING", "blocking_for_generation": [], "blocking_for_mapdl_launch": ["C10-NUM-01", "C10-RANK-01"], "blocking_for_production_use": ["C10-DWG-01", "C10-NUM-01", "C10-RANK-01"], "inherited_parent_limitations": parent_status.get("documented_limitations", [])}  # 区分生成、启动和生产使用三个门槛。
