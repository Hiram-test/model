...
DISK_STOP_BYTES = 32 * 1024**3  # 运行盘空余低于三十二 GiB 时提交官方 ABT以保留写盘余量。
...
def validate_launch_chain(run_dir: Path) -> dict[str, Any]:  # 输入 A0 运行目录并返回已闭合的启动链、路径与进程身份对象。
    ...
    require(manifest.get("run_name") == resolved_run.name == status.get("run_name") == claim.get("run_name") == launch.get("run_name"), "A0 清单、状态、认领或启动记录运行名不一致")  # 关闭跨目录复制错配。
    require(manifest.get("jobname") == status.get("jobname") == claim.get("jobname") == launch.get("jobname"), "A0 启动链 jobname 不一致")  # 固定唯一结果文件族。
    ...
    return {"run_dir": resolved_run, "manifest": manifest, "claim_path": claim_path, "launch_path": launch_path, "launch": launch, "identity_path": identity_path, "identity": identity, "solver_dir": solver_dir, "jobname": jobname, "output_path": output_path}  # 返回持续监控所需且已闭合的最小对象集合。
...
def offline_self_test() -> dict[str, Any]:  # 无业务输入；只在内存和系统临时目录验证帮助模块、安全源码和硬门常量。
    ...
    return {"schema_version": 1, "status": "PASSED", "base_monitor_self_test": base_result, "expected_equation_count": EXPECTED_EQUATION_COUNT, "forbidden_process_action_count": 0, "comment_violation_count": 0, "native_abort_only": True, "force_termination_allowed": False, "mapdl_execution_attempted": False, "mapdl_started": False}  # 返回安全帮助函数、方程门、源码和零启动自检摘要。
...
def monitor(validated: dict[str, Any]) -> dict[str, Any]:  # 输入已闭合启动链并持续监控至稳定退出，返回最终状态摘要。
    ...
    claim = {"schema_version": 1, "status": "A0_MONITOR_CLAIMED", "run_name": run_dir.name, "jobname": jobname, "claimed_at_utc": utc_now(), "monitor_script": str(Path(__file__).resolve()), "monitor_script_sha256": monitor_base.sha256_file(Path(__file__).resolve()), "runtime_launch_sha256": monitor_base.sha256_file(validated["launch_path"]), "runtime_launch_claim_sha256": monitor_base.sha256_file(validated["claim_path"]), "runtime_process_identity_sha256": monitor_base.sha256_file(validated["identity_path"]), "manifest_sha256": monitor_base.sha256_file(run_dir / "manifest.json"), "prepared_ledger_sha256": monitor_base.sha256_file(run_dir / "artifact_hashes.sha256"), "initial_related_processes": initial_related, "initial_unrelated_solver_processes": initial_unrelated, "expected_equation_count": EXPECTED_EQUATION_COUNT, "hard_stops": validated["launch"]["monitor_hard_stops"], "native_abort_payload_sha256": monitor_base.NATIVE_ABORT_PAYLOAD_SHA256, "native_abort_only": True, "force_termination_allowed": False, "default_cnvtol_relaxation_messages_are_hard_events": True}  # 冻结监控身份、启动链、进程、方程、资源和默认 CNVTOL 日志硬门。
    ...
    with samples_path.open("x", encoding="utf-8", newline="\n") as samples_handle:  # 排他创建逐样本 JSONL，禁止第二监控器覆盖。
        while True:  # 持续采样直至自然或 ABT 后进程树稳定退出，或退出证据超时阻断。
            ...
            new_events = output_events + error_events  # 合并本轮日志硬事件并保持发现顺序。
            ...
            if disk_free < DISK_STOP_BYTES and "RESOURCE_DISK_BELOW_32_GIB" not in seen_event_keys:  # 首次磁盘低于三十二 GiB 时触发写盘安全硬门。
                seen_event_keys.add("RESOURCE_DISK_BELOW_32_GIB")  # 登记去重键。
                new_events.append({"kind": "RESOURCE_DISK_BELOW_32_GIB", "observed_bytes": disk_free, "threshold_bytes": DISK_STOP_BYTES, "detected_at_utc": utc_now()})  # 保存实测、阈值与时刻。
    ...
    final_record = {"schema_version": 1, "status": final_status, "run_name": run_dir.name, "jobname": jobname, "monitor_started_at_utc": started_at_utc, "monitor_finished_at_utc": utc_now(), "duration_seconds": time.monotonic() - started_monotonic, "sample_count": sample_count, "monitor_claim_sha256": monitor_base.sha256_file(monitor_claim_path), "monitor_block_reason": monitor_block_reason, "minimum_physical_memory_available_bytes": low_ram_minimum, "minimum_disk_free_bytes": disk_minimum, "maximum_related_rss_bytes": maximum_related_rss, "observed_equation_counts": observed_equations, "unique_equation_counts": sorted(set(observed_equations)), "expected_equation_count": EXPECTED_EQUATION_COUNT, "hard_events": hard_events, "native_abort": abort_evidence, "terminate_called": False, "kill_called": False, "send_signal_called": False, "final_related_processes": final_related, "final_unrelated_solver_processes": final_unrelated, "final_out_file": monitor_base.file_snapshot(output_path), "final_err_file": monitor_base.file_snapshot(error_path), "final_mntr_file": monitor_base.file_snapshot(native_monitor_path), "final_lock_file": monitor_base.file_snapshot(lock_path), "samples_path": str(samples_path), "samples_sha256": monitor_base.sha256_file(samples_path), "result_use": "A0_EXECUTION_MONITOR_ONLY_EXTERNAL_STATIC_QA_REQUIRED_NO_MODAL_OR_PRODUCTION_CLAIM"}  # 汇总身份、资源、方程数、硬事件、官方 ABT、退出证据和严格用途边界。
    monitor_base.write_json_exclusive(final_path, final_record)  # 以排他创建提交唯一监控终态。
    return {"run_dir": str(run_dir), "status": final_status, "sample_count": sample_count, "hard_event_count": len(hard_events), "native_abort_requested": bool(abort_evidence.get("request_contract_satisfied")), "terminate_called": False, "kill_called": False, "external_static_qa_required": True}  # 返回紧凑机器摘要供后续终结器或人工接管。
