...
def build_unit_test_plan(unit_test_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:  # 输入目标目录并生成 30 份未执行微模型和计划/结果摘要。
    ...
    plan["acceptance"] = {"micro_case": {"max_relative_translation_slip_mm": 1.0e-5, "force_equilibrium_relative_tolerance": 1.0e-6, "moment_equilibrium_relative_tolerance": 1.0e-6, "unit_moment_master_transfer_ratio_max": 1.0e-6, "weak_spring_reaction_must_be_reported_separately": True}, "full_bridge_follow_on": {"adjacent_penalty_same_physical_mode_frequency_change_percent": 0.05, "matrix_coefficient_ratio_degradation_factor_max": 10.0, "covered_by_thirty_micro_cases": False}}  # 把 30 个微算例可直接判定的门禁与必须留给全桥的模态/矩阵门禁明确分层。
