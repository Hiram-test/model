...
def make_single_mpc_uxyz_load_test_input(case_id: str, load_label: str, load_value: float) -> str:  # 输入平移载荷案例并返回生产拓扑单 TYPE72 装配秩微模型。
    ...
    lines = [  # 初始化生产拓扑微模型 APDL 行表。
        ...
        "CNVTOL,F,0.005,1.0,2 ! 固定 0.5% 力收敛阈值和 1 N 最小参考值。",  # 禁止求解器自动放宽力判据。
        "CNVTOL,U,0.05,,2 ! 固定 5% 位移增量收敛阈值。",  # 禁止自动收敛容差重置。
...
def make_single_mpc_finite_rotation_test_input(axis_name: str, rotation_label: str, rotation_value_rad: float) -> str:  # 输入转轴和规定转角并返回单 TYPE72 有限转动测试。
    """用单 TYPE72 加双侧纯平移 LINK180 验证 0.1 rad Rodrigues 偏心平移，不再引入辅助节点或 BEAM slave。"""  # 返回 LF 文本。
    ...
    rotation_values = {label: rotation_value_rad if label == rotation_label else 0.0 for label in ("ROTX", "ROTY", "ROTZ")}  # 只给目标轴赋 0.1 rad，其余轴固定为零。
    lines = [  # 初始化有限转动微模型 APDL 行表。
        "! C10 单 TYPE72 0.1 rad 有限转动解析测试；本文件由 prepare 生成但不自动执行。",  # 说明案例目的和执行边界。
        ...
        "N,1,0,0,0 ! 定义规定转动的 master。",  # master 位于原点。
        ...
        f"D,1,ROTX,{format(rotation_values['ROTX'], '.15g')} ! 规定 master 绕 X 转角，单位 rad。",  # 当前轴为 0.1，其余为零。
        f"D,1,ROTY,{format(rotation_values['ROTY'], '.15g')} ! 规定 master 绕 Y 转角，单位 rad。",  # 当前轴为 0.1，其余为零。
        f"D,1,ROTZ,{format(rotation_values['ROTZ'], '.15g')} ! 规定 master 绕 Z 转角，单位 rad。",  # 当前轴为 0.1，其余为零。
        ...
        "CNVTOL,F,0.005,1.0,2 ! 固定力收敛阈值和最小参考值。",  # 禁止自动容差放宽。
        "CNVTOL,U,0.05,,2 ! 固定 5% 位移增量阈值。",  # 禁止自动容差重置。
        "OUTRES,ALL,ALL ! 保存全部二十个子步结果。",  # 支持路径和方程数复核。
        "SOLVE ! 执行当前有限转动测试；prepare 本身不调用本文件。",  # 求解由后续执行器负责。
        "FINISH ! 结束求解阶段。",  # 切换到后处理。
        "/POST1 ! 进入结果后处理器。",  # 读取终态。
        "SET,LAST ! 读取 0.1 rad 终态结果。",  # 固定结果集。
...
def build_unit_test_plan(unit_test_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:  # 输入目标目录并生成 12 份未执行微模型和计划/结果摘要。
    ...
    plan = {"schema_version": 3, "status": "PREPARED_NOT_STARTED", "purpose": "C10_SINGLE_MPC_PRODUCTION_TOPOLOGY_RANK_KINEMATICS_AND_TRANSFER", "planned_case_count": len(cases), "completed_case_count": 0, "mapdl_execution_attempted": False, "formulation": {"UXYZ": "SINGLE_TYPE72_PROJECTED_TO_TRANSLATION_ONLY_SLAVE", "ALL": "TYPE72_RIGID_BEAM", "constraint_imposition": "DIRECT_ELIMINATION", "aux_node_count": 0, "type73_count": 0, "penalty_factor": None}, "acceptance": {"max_rigid_body_translation_error_mm": 1.0e-6, "finite_rotation_rodrigues_displacement_error_mm": 1.0e-6, "force_equilibrium_absolute_tolerance_n": 1.0e-6, "moment_equilibrium_absolute_tolerance_n_mm": 1.0e-6, "all_slave_translation_and_rotation_max_abs": 1.0e-10, "automatic_cnvtol_reset_allowed": False, "solver_error_allowed": False, "zero_small_or_negative_pivot_allowed": False, "equation_count_change_within_case_allowed": False}, "cases": cases}  # 汇总十二个无罚函数、无串联链硬门禁案例及数值阈值。
