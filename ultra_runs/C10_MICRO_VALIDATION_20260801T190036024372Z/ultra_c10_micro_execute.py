...
ERROR_COUNT_PATTERN = re.compile(r"NUMBER OF ERROR\s+MESSAGES ENCOUNTERED\s*=\s*(\d+)", re.IGNORECASE)  # 允许 ERROR 与 MESSAGES 间的 MAPDL 对齐空格并提取页尾累计错误数。
WARNING_COUNT_PATTERN = re.compile(r"NUMBER OF WARNING\s+MESSAGES ENCOUNTERED\s*=\s*(\d+)", re.IGNORECASE)  # 允许 WARNING 与 MESSAGES 间的对齐空格并提取页尾累计警告数。
