...
def evaluate_sections(run_dir: Path) -> dict[str, Any]:  # 定义六截面判门函数，输入套件目录，输出属性、剪切映射与柔度指标。
    """要求六类 ASEC 的 A/Iyy/Izz/J 误差小于 0.1%，原生剪切映射闭合且短梁柔度误差小于 0.5%。"""  # 函数说明给出三组门槛。

    properties = parse_numeric_csv(run_dir / "post" / "sections__u01_section_properties.csv")  # 读取六行十三列原生/ASEC 属性。
    deflections = parse_numeric_csv(run_dir / "post" / "sections__u01_section_deflections.csv")  # 读取六行七列短梁结果。
    ...
    if len(deflections) != 6 or any(len(row) != 7 for row in deflections):  # 要求六类双向短梁结果全部闭合。
        raise RuntimeError(f"截面柔度结果形状错误：rows={len(deflections)}")  # 报告行数并停止判门。
    ...
    deflection_errors: list[float] = []  # 创建由 MAPDL 位移与解析位移独立重算的十二项相对误差列表，不信任 APDL 自报误差列。
    reported_deflection_error_differences: list[float] = []  # 创建 APDL 自报误差与 Python 重算误差的差值列表，用于发现模板列错位或硬编码。
    for index, row in enumerate(deflections, start=1):  # 按预注册六类截面顺序逐行重算双向短梁柔度误差。
        if row[0] != float(index):  # 要求柔度 CSV 第一列精确等于 1 至 6，禁止重复、乱序或非整数编号被按行位置接受。
            raise RuntimeError(f"截面柔度序号不连续：row={row}")  # 报告整行数据并停止判门，避免不同截面的数值与目标错配。
        ...
        deflection_errors.extend([recomputed_y_error, recomputed_z_error])  # 按截面和方向顺序保存两项独立误差，供 0.5% 硬门禁取最大值。
        reported_deflection_error_differences.extend([abs(row[3] - recomputed_y_error), abs(row[6] - recomputed_z_error)])  # 比较 APDL 自报两列与 Python 重算值，防止错误列仍给出虚假小误差。
    ...
    flexibility_pass = max(deflection_errors) < 0.005  # 要求十二项短梁 Timoshenko 柔度误差严格小于 0.5%。
    reported_error_consistency_threshold = 1.0e-10  # 允许 APDL 与 Python 双精度格式化带来至多 1E-10 的无量纲误差差值，远小于 0.5% 主门槛。
    reported_error_consistency_pass = max(reported_deflection_error_differences) <= reported_error_consistency_threshold  # 要求十二项 APDL 自报误差与独立重算值全部一致。
    passed = property_pass and shear_mapping_pass and flexibility_pass and reported_error_consistency_pass  # 属性、剪切映射、独立柔度和自报列一致性四组门禁必须同时通过。
    return {"test_id": "U01_03_SECTIONS", "passed": passed, "maximum_property_relative_error": max(property_errors), "property_threshold": 0.001, "maximum_shear_mapping_relative_error": max(shear_errors), "shear_threshold": 0.005, "maximum_deflection_relative_error": max(deflection_errors), "deflection_threshold": 0.005, "maximum_reported_deflection_error_difference": max(reported_deflection_error_differences), "reported_error_consistency_threshold": reported_error_consistency_threshold, "maximum_native_torsion_difference_from_frozen_target": max(native_torsion_differences), "native_sections_role": "REFERENCE_ONLY_NOT_AUTOMATIC_PRODUCTION"}  # 返回四组判据、独立重算一致性及原生 J 风险。
...
def prepare_cases(run_dir: Path, stamp: str, suffix: str) -> list[dict[str, Any]]:  # 定义八个执行案例准备函数。
    ...
    specifications = [  # 按执行顺序定义案例标识、构造器、并行模式和预注册结果文件。
        ...
        {"case_id": "sections", "builder": lambda name: build_template_input("u01_sections.inp", name), "parallel_mode": "SERIAL", "result_files": ["u01_section_properties.csv", "u01_section_deflections.csv"], "purpose": "six ASEC/native properties and short-beam shear"},  # 六截面属性和短梁案例。
