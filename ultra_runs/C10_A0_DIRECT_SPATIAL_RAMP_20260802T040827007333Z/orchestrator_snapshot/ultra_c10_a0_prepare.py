...
DISK_GATE_GIB = 24  # 冻结 A0 启动和运行监控统一最小空盘门为二十四 GiB，单位 GiB。
DISK_GATE_BYTES = DISK_GATE_GIB * 1024**3  # 把二十四 GiB 按二进制 GiB 换算为字节供机器清单和工具合同复核。
DISK_GATE_MULTIPLIER = 4.26  # 冻结二十四 GiB 相对 5.630 GiB 历史最大包约四点二六倍的无量纲安全倍率。
REFERENCE_FREE_DISK_GIB = 38.653  # 冻结本轮门限决策时 D 盘实测空余 38.653 GiB，单位 GiB。
REFERENCE_EXTRA_MARGIN_GIB = 14.653  # 冻结 38.653 GiB 减二十四 GiB 后约 14.653 GiB 的启动时额外余量，单位 GiB。
HOST_LEASE_PATH = RUNS_ROOT / ".c10_a0_exclusive_host_lease.json"  # 冻结未来 A0 启动器在第一次空扫描后取得、监控器稳定终态释放的唯一跨运行主机租约路径。
MONITOR_ATTACHMENT_TIMEOUT_SECONDS = 30.0  # 冻结启动器自动创建专用监控器后等待有效 claim 的最长三十秒窗口，单位 s。
...
def validate_prepared_run(run_dir: Path) -> dict[str, Any]:  # 输入唯一 A0 运行目录并返回不写盘的全包复核结果。
    ...
    resource_gate = manifest.get("resource_gate", {})  # 读取机器清单中的统一启动与运行资源门对象。
    require(resource_gate.get("minimum_free_disk_gib") == DISK_GATE_GIB and resource_gate.get("minimum_free_disk_bytes") == DISK_GATE_BYTES and resource_gate.get("historical_largest_c10_package_gib") == HISTORICAL_LARGEST_C10_PACKAGE_GIB, "A0 清单不是统一二十四 GiB 历史证据磁盘门")  # 阻断废弃五十 GiB 包或低于批准门的清单。
    orchestration = manifest.get("runtime_orchestration", {})  # 读取全局租约、双扫描、自动监控握手与并发硬事件机器合同。
    require(orchestration.get("host_lease_path") == str(HOST_LEASE_PATH) and orchestration.get("double_solver_scan_required") is True and orchestration.get("monitor_auto_start_required") is True and orchestration.get("monitor_claim_timeout_seconds") == MONITOR_ATTACHMENT_TIMEOUT_SECONDS and orchestration.get("postlaunch_unrelated_solver_process_is_hard_event") is True and orchestration.get("blocked_or_unconfirmed_exit_retains_host_lease") is True and orchestration.get("active_process_disposition_allowed") is False, "A0 清单 P1 独占、自动监控或阻断保留合同不完整")  # 固定启动前后扫描、自动附着、无关进程硬门和不确定终态租约保留边界。
    ...
    forbidden_runtime_names = ["runtime_launch_claim.json", "runtime_launch.json", "runtime_process_identity.json", "runtime_process_identity_failure.json", "runtime_monitor_spawn_claim.json", "runtime_monitor_launch.json", "runtime_monitor_attachment.json", "runtime_monitor_attachment_failure.json", "runtime_prelaunch_failure.json", "runtime_popen_failure.json"]  # 冻结任何实际启动、自动监控或故障路径都会产生的根级证据文件名。
...
def generate_run() -> Path:  # 无业务输入；生成一个唯一 A0 可执行准备包并返回其绝对目录。
    ...
    status = {"schema_version": 1, "run_name": run_name, "jobname": jobname, "status": "STATIC_DIAGNOSTIC_PREPARED", "diagnostic_subtype": DIAGNOSTIC_SUBTYPE, "created_at_utc": created_at.isoformat(), "parent_run": C10_RUN_NAME, "successful_control_reference_run": S10_RUN_NAME, "convergence_acceptance_scope": "DEFAULT_CNVTOL_DIAGNOSTIC_ONLY", "mapdl_execution_attempted": False, "mapdl_started": False, "execution_mode": "SMP_SERIAL_NP1_DIAGNOSTIC_ONLY", "resource_gate": "DIAGNOSTIC_4_GIB_RAM_AND_EVIDENCE_BASED_24_GIB_DISK_WITH_RUNTIME_RECHECK", "launch_allowed_for_diagnostic": True, "launch_allowed_for_production": False, "full_bridge_static_status": "A0_PREPARED_NOT_STARTED", "full_bridge_modal_status": "NOT_RUN", "valid_for_production": False, "next_action": "EXPLICITLY_RUN_FROZEN_A0_EXECUTE_VALIDATE_ONLY_BEFORE_ANY_SEPARATE_LAUNCH_AUTHORIZATION"}  # 冻结准备态、默认 CNVTOL 历史二分用途、四 GiB 内存和二十四 GiB 证据化磁盘门、零启动及下一动作边界。
    manifest = {"schema_version": 1, "run_name": run_name, "jobname": jobname, "status": status["status"], "diagnostic_subtype": DIAGNOSTIC_SUBTYPE, "created_at_utc": created_at.isoformat(), "parent_run": C10_RUN_NAME, "successful_control_reference_run": S10_RUN_NAME, "constraint_topology": "SINGLE_TYPE72_NO_AUX_NO_TYPE73", "constraint_imposition": "DIRECT_ELIMINATION", "expected_topology": parent_manifest["expected_topology"], "expected_equation_count": EXPECTED_EQUATION_COUNT, "main_input": f"solver/{A0_MAIN_NAME}", "main_input_sha256": transform_audit["candidate_sha256"], "controlled_include": f"solver/{C10_GATE_NAME}", "controlled_include_sha256": C10_GATE_SHA256, "dependencies": dependency_audit, "load_path_mode": "S10_DIRECT_SPATIAL_MASS21_GRAVITY_RAMP_NO_MIGRATION", "ls1": transform_audit["ls1"], "ls2": transform_audit["ls2"], "nrre": {"enabled": True, "label": "NRRE", "maxfile": NRRE_MAXFILE, "eflg_enabled": False}, "convergence_acceptance_scope": "DEFAULT_CNVTOL_DIAGNOSTIC_ONLY", "default_cnvtol_log_auto_relaxation_allowed": False, "cnvtol_explicit_command_count": 0, "cnvtol_changed": False, "if_a0_passes_next_run": "A0B_SAME_PATH_WITH_CURRENT_FOUR_EXPLICIT_CNVTOL_STATIC_ACCEPTANCE", "a0_direct_production_promotion_allowed": False, "mpc184_keyopt5_static": 0, "keyopt_changed": False, "initial_state_changed": False, "mass_changed": False, "load_changed": False, "modal_requested": False, "production_claim_allowed": False, "static_result_expected": True, "runtime_equation_count_change_allowed": False, "runtime_small_zero_negative_pivot_allowed": False, "resource_gate": {"minimum_available_ram_bytes": 4 * 1024**3, "minimum_free_disk_bytes": DISK_GATE_BYTES, "minimum_free_disk_gib": DISK_GATE_GIB, "historical_largest_c10_package_gib": HISTORICAL_LARGEST_C10_PACKAGE_GIB, "disk_gate_multiplier": DISK_GATE_MULTIPLIER, "reference_free_disk_gib": REFERENCE_FREE_DISK_GIB, "reference_extra_margin_gib": REFERENCE_EXTRA_MARGIN_GIB, "runtime_recheck_required": True}, "mapdl_executable": str(MAPDL_EXE), "mapdl_executable_sha256": static_base.sha256_file(MAPDL_EXE), "execution_mode": status["execution_mode"], "launch_argv": launch_argv, "runtime_execute_script": f"input_snapshot/{EXECUTE_TOOL_NAME}", "runtime_execute_script_sha256": execute_hash, "runtime_monitor_script": f"input_snapshot/{MONITOR_TOOL_NAME}", "runtime_monitor_script_sha256": monitor_hash, "runtime_static_helper_script": f"input_snapshot/{STATIC_HELPER_NAME}", "runtime_static_helper_script_sha256": static_helper_hash, "runtime_monitor_helper_script": f"input_snapshot/{MONITOR_HELPER_NAME}", "runtime_monitor_helper_script_sha256": monitor_helper_hash}  # 汇总唯一输入、默认 CNVTOL 诊断边界、A0b 后续验收义务、二十四 GiB 磁盘门证据、禁改项、运行工具与求解器身份。
    manifest["runtime_orchestration"] = {"host_lease_path": str(HOST_LEASE_PATH), "host_lease_acquired_after_first_solver_scan": True, "double_solver_scan_required": True, "monitor_auto_start_required": True, "monitor_claim_timeout_seconds": MONITOR_ATTACHMENT_TIMEOUT_SECONDS, "monitor_releases_host_lease_only_after_stable_empty_exit": True, "blocked_or_unconfirmed_exit_retains_host_lease": True, "runtime_failure_exit_timeout_seconds": 600.0, "postlaunch_unrelated_solver_process_is_hard_event": True, "native_abort_target_scope": "THIS_A0_JOB_ONLY", "active_process_disposition_allowed": False}  # 冻结租约、双扫描、自动监控、十分钟完整退出门、阻断保留和只对本 job 写 ABT 的 P1 机器合同。
    ...
    zero_start = {"schema_version": 1, "status": "PASSED_ZERO_START", "run_name": run_name, "jobname": jobname, "observed_at_utc": datetime.now(timezone.utc).isoformat(), "preparer_script": str(Path(__file__).resolve()), "preparer_script_sha256": static_base.sha256_file(Path(__file__).resolve()), "preparer_subprocess_import": False, "preparer_process_start_call": False, "launcher_launch_mode_executed": False, "monitor_live_mode_executed": False, "runtime_launch_claim_exists": False, "runtime_launch_exists": False, "runtime_process_identity_exists": False, "runtime_monitor_spawn_claim_exists": False, "runtime_monitor_launch_exists": False, "runtime_monitor_attachment_exists": False, "global_host_lease_exists": HOST_LEASE_PATH.exists(), "job_output_files_present": False, "mapdl_execution_attempted": False, "mapdl_started": False, "static_result": "NOT_RUN", "modal_result": "NOT_RUN"}  # 冻结代码路径、启动与监控模式、全部握手工件、全局租约和结果状态零启动证明。
    ...
    launch_contract = f"# A0 启动与监控合同\n\n当前状态：准备完成、ANSYS 未启动。\n\n只读复核：`python -B input_snapshot/{EXECUTE_TOOL_NAME} --run-dir <本运行绝对目录> --validate-only`。该命令不得生成任何 runtime 或 solver 输出。\n\n若后续另行明确授权实际启动，只使用同一冻结启动器的 `--launch`；启动器会在租约前后执行两次 MAPDL/MPI 空扫描、取得唯一全局主机租约、启动 MAPDL、自动启动冻结专用监控器，并在 {MONITOR_ATTACHMENT_TIMEOUT_SECONDS:.0f} 秒内等待有效 claim，禁止人工另行附着。启动后任何非本 job MAPDL/MPI 都是硬事件，只允许向本 A0 job 写精确十字节 `nonlinear\\n` ABT 并等待自然退出；绝不 terminate、kill、发信号或处置其他进程。启动器只允许 SMP1、唯一 job、至少四 GiB 可用物理内存和至少 {DISK_GATE_GIB} GiB 空盘；磁盘门来自历史最大 C10 失败/NRRE 包 {HISTORICAL_LARGEST_C10_PACKAGE_GIB:.3f} GiB 的约 {DISK_GATE_MULTIPLIER:.2f} 倍。只有本 job 进程为空、lock 消失且 OUT/ERR/MNTR 连续稳定才释放租约；阻断或十分钟未闭合时保留租约供人工审计并阻止后继运行。\n"  # 生成人读但不执行的自动监控、租约双扫描、统一二十四 GiB 门、完整退出门和无强制处置合同。
    (qa_dir / "launch_contract.md").write_text(launch_contract, encoding="utf-8", newline="\n")  # 写出未来授权动作的唯一顺序与安全要求。
    result_packet = f"# C10 A0 直接空间重力斜坡短验证准备结果\n\n状态：`STATIC_DIAGNOSTIC_PREPARED`；ANSYS 未启动。\n\n- 运行：`{run_name}`；job：`{jobname}`。\n- 当前连接：5,078 个单层 TYPE72，`KEYOPT(1/2/5)=1/0/0`；辅助节点=0，TYPE73=0。\n- 荷载路径：恢复 S10 的直接空间 MASS21 重力斜坡；LS1 为 `KBC,0 / AUTOTS,ON / NSUBST,20,200,20 / PRED,OFF`，LS2 为零增量保持。\n- 唯一新增诊断：`NLDIAG,NRRE,ON,50`；未增加或修改 CNVTOL、KEYOPT、初始状态、质量、荷载、PRED 或 NEQIT。\n- 收敛用途：`DEFAULT_CNVTOL_DIAGNOSTIC_ONLY`；日志中 CNVTOL 被忽略或内部自动放宽属于硬事件。A0 即使通过也不得直接升格正式修复，必须再做同路径且恢复当前四项显式 CNVTOL 的 A0b 静力验收。\n- 资源门：启动和监控统一为 {DISK_GATE_GIB} GiB 空盘；依据为历史最大 C10 失败/NRRE 包 {HISTORICAL_LARGEST_C10_PACKAGE_GIB:.3f} GiB 的约 {DISK_GATE_MULTIPLIER:.2f} 倍，本轮参考空余下另留约 {REFERENCE_EXTRA_MARGIN_GIB:.3f} GiB。\n- 分析范围：装配、两步静力和全部静力门；模态命令已完整截断。\n- 当前没有静力结果、模态结果或生产结论。A0 若通过，先做 A0b 而不做荷载位置迁移；A0 若失败，才进入分组件迁移定位。\n"  # 生成便于人工快速确认的准备结论、默认 CNVTOL 边界、A0b 义务、二十四 GiB 资源证据和后续判定逻辑。
