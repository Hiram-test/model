...
LS1_HISTORY_FILE_STEM = "b00_ls1_energy_history"  # LS1 每个已保存收敛子步的 SENE、STEN 与比值使用该固定 CSV 文件干名。
...
def build_main_input(jobname: str, modes: int, dependencies: list[dict[str, Any]]) -> str:  # 定义完整 B00 静力—扰动模态主输入构造函数。
    ...
    add_apdl(lines, "OUTRES,ALL,LAST", "仅保存 LS1 末完整场，控制全桥位移、应力和反力结果体积。")  # 设置 LS1 非能量结果输出。
    add_apdl(lines, "OUTRES,VENG,ALL", "保存 LS1 每个已收敛子步的 SENE/STEN，供任务书要求的全历程峰值门禁。")  # 保存全历程能量结果。
    ...
    lines.append("! LS1 收敛门禁通过后保持在同一 /SOLU 分析中，使下一次 SOLVE 自动形成真实载荷步 2。")  # 生成只读 APDL 说明并避免 FINISH 后重新 /SOLU 把第二次求解重置为载荷步 1。
    ...
    add_apdl(lines, "SET,LAST", "先激活静力 RST 最后一个结果集，供 NSET 读取全部已保存结果集数量。")  # 建立全历程遍历身份。
    add_apdl(lines, "*GET,B00_HNSET,ACTIVE,0,SET,NSET", "读取静力 RST 中全部已保存结果集数量，包含 LS1 能量子步和 LS2 端点。")  # 获取全历程结果集总数。
    add_apdl(lines, "B00_HCOUNT=0", "初始化 LS1 已保存收敛子步计数；遍历后必须严格大于零。")  # 初始化 LS1 历程计数。
    add_apdl(lines, "B00_PEAK1=-1", "初始化 LS1 全历程绝对 STEN/SENE 峰值为负哨兵，任何合法非负比值都会替换它。")  # 初始化峰值。
    add_apdl(lines, "B00_PEAKSB=-1", "初始化 LS1 峰值所在子步号为负哨兵，单位为无量纲整数序号。")  # 初始化峰值子步。
    add_apdl(lines, "B00_PEAKTM=-1", "初始化 LS1 峰值所在伪时间为负哨兵，合法范围为零至一秒制伪时间。")  # 初始化峰值时间。
    add_apdl(lines, f"*CFOPEN,{LS1_HISTORY_FILE_STEM},csv", "打开无标题纯数值 LS1 全历程 CSV；正常与拒绝路径均在峰值门禁前关闭。")  # 打开 LS1 历程输出。
    add_apdl(lines, "SET,FIRST", "激活静力 RST 的第一个已保存结果集，按真实写入顺序开始遍历。")  # 定位首个结果集。
    add_apdl(lines, "*DO,B00_HI,1,B00_HNSET", "逐一遍历静力 RST 的全部结果集，并只统计载荷步一的已收敛能量记录。")  # 开始全历程循环。
    add_apdl(lines, "*GET,B00_HLS,ACTIVE,0,SET,LSTP", "读取当前结果集载荷步编号，用于排除 LS2 保持步。")  # 获取当前载荷步。
    add_apdl(lines, "*GET,B00_HSB,ACTIVE,0,SET,SBST", "读取当前结果集子步编号，供 CSV 与峰值位置追溯。")  # 获取当前子步。
    add_apdl(lines, "*GET,B00_HTIME,ACTIVE,0,SET,TIME", "读取当前结果集伪时间，供全历程横坐标与峰值位置追溯。")  # 获取当前时间。
    add_apdl(lines, "*IF,B00_HLS,EQ,1,THEN", "只有载荷步一结果集进入 LS1 能量统计，LS2 由独立端点门禁处理。")  # 开始 LS1 过滤分支。
    add_apdl(lines, "B00_HCOUNT=B00_HCOUNT+1", "把当前 LS1 已保存收敛子步加入全历程计数。")  # 累加 LS1 历程计数。
    add_apdl(lines, "ETABLE,ERAS", "清空上一结果集的单元表定义，防止能量列跨子步残留。")  # 清空历程 ETABLE。
    add_apdl(lines, "ETABLE,B0HSENE,SENE", "建立当前 LS1 子步的全单元势能列，单位 N·mm。")  # 建立历程 SENE 列。
    add_apdl(lines, "ETABLE,B0HSTEN,STEN", "建立当前 LS1 子步的全单元稳定化能列，单位 N·mm。")  # 建立历程 STEN 列。
    add_apdl(lines, "SSUM", "对当前 LS1 子步的 SENE 与 STEN 单元列分别求全模型总和。")  # 汇总当前历程能量。
    add_apdl(lines, "*GET,B00_HSENE,SSUM,0,ITEM,B0HSENE", "读取当前 LS1 子步全模型势能总和，单位 N·mm。")  # 获取历程 SENE。
    add_apdl(lines, "*GET,B00_HSTEN,SSUM,0,ITEM,B0HSTEN", "读取当前 LS1 子步全模型稳定化能总和，单位 N·mm。")  # 获取历程 STEN。
    add_apdl(lines, "B00_HDEN=ABS(B00_HSENE)", "以当前 LS1 子步势能绝对值构造稳定化能量比分母。")  # 计算历程分母。
    add_apdl(lines, "*IF,B00_HDEN,LT,1.0E-30,THEN", "当前 LS1 子步势能绝对值过小时进入 1E-30 N·mm 除零保护。")  # 开始历程分母保护。
    add_apdl(lines, "B00_HDEN=1.0E-30", "把当前 LS1 子步数值保护分母设为 1E-30 N·mm。")  # 设置历程保护分母。
    add_apdl(lines, "*ENDIF", "结束当前 LS1 子步势能分母保护。")  # 结束历程分母保护。
    add_apdl(lines, "B00_HRATIO=ABS(B00_HSTEN)/B00_HDEN", "计算当前 LS1 子步绝对 STEN/SENE 无量纲比值。")  # 计算历程能量比。
    add_vwrite(lines, "*VWRITE,B00_HLS,B00_HSB,B00_HTIME,B00_HSENE,B00_HSTEN,B00_HRATIO", "(F12.0,',',F12.0,',',E24.16,',',E24.16,',',E24.16,',',E24.16)", "写出载荷步、子步、伪时间、SENE、STEN 和绝对比值六列纯数值记录。")  # 输出当前 LS1 历程行。
    add_apdl(lines, "*IF,B00_HRATIO,GT,B00_PEAK1,THEN", "当前 LS1 子步比值大于既有峰值时更新峰值及其位置。")  # 开始峰值更新分支。
    add_apdl(lines, "B00_PEAK1=B00_HRATIO", "保存新的 LS1 全历程绝对 STEN/SENE 峰值。")  # 更新峰值。
    add_apdl(lines, "B00_PEAKSB=B00_HSB", "保存新峰值所在 LS1 子步号。")  # 更新峰值子步。
    add_apdl(lines, "B00_PEAKTM=B00_HTIME", "保存新峰值所在 LS1 伪时间。")  # 更新峰值时间。
    add_apdl(lines, "*ENDIF", "结束当前 LS1 子步峰值更新分支。")  # 结束峰值更新。
    add_apdl(lines, "*ENDIF", "结束载荷步一过滤分支；非 LS1 结果集不写入历程 CSV。")  # 结束 LS1 过滤。
    add_apdl(lines, "*IF,B00_HI,LT,B00_HNSET,THEN", "尚未遍历最后一个结果集时才请求下一结果集，避免末尾越界警告。")  # 开始结果集推进保护。
    add_apdl(lines, "SET,NEXT", "按静力 RST 原生顺序读取下一已保存结果集。")  # 推进全历程结果集。
    add_apdl(lines, "*ENDIF", "结束结果集推进保护。")  # 结束推进保护。
    add_apdl(lines, "*ENDDO", "结束全部静力结果集遍历，LS1 全历程峰值和 CSV 已在内存与磁盘形成。")  # 结束全历程循环。
    add_apdl(lines, "*CFCLOS", "关闭并刷新 LS1 全历程 CSV，确保后续任何 fail-closed 退出仍保留完整历程。")  # 关闭 LS1 历程输出。
    ...
    add_vwrite(lines, "*VWRITE,B00_HNSET,B00_HCOUNT,B00_PEAKSB,B00_PEAKTM", "('STATIC_NSET=',F12.0,', LS1_HISTORY_COUNT=',F12.0,', PEAK_SUBSTEP=',F12.0,', PEAK_TIME=',E24.16)", "写出静力结果集总数、LS1 历程记录数以及峰值所在子步和伪时间。")  # 输出历程身份与峰值位置。
    add_vwrite(lines, "*VWRITE,B00_PEAK1", "('LS1_HISTORY_PEAK_ABS_STEN_OVER_SENE=',E24.16)", "写出任务书要求的 LS1 全历程绝对 STEN/SENE 峰值。")  # 输出历程峰值。
    ...
    add_vwrite(lines, "*VWRITE,B00_RFEXP,B00_RFZ", "('RF_EXPECTED=',E24.16,', RF_ACTUAL=',E24.16)", "写出理论重力反力与实际支反力，两个值的单位均为 N；分行可保持单条物理记录短小且易解析。")  # 输出反力基准和实测值。
    add_vwrite(lines, "*VWRITE,B00_RFERR,B00_RFREL", "('RF_ERROR=',E24.16,', RF_RELATIVE_ERROR=',E24.16)", "写出支反力有符号误差与无量纲相对误差；与上一行共同组成完整反力闭合证据。")  # 输出反力误差和相对误差。
    ...
    add_reject_gate(lines, "B00_HCOUNT", "LE", "0", "LS1_ENERGY_HISTORY_EMPTY", "LS1 没有任何已保存能量结果集时拒绝，禁止用端点替代全历程门禁。")  # LS1 历程存在性门禁。
    ...
    add_reject_gate(lines, "B00_PEAK1", "GT", f"{LS1_ENERGY_RATIO_LIMIT:.1E}", "LS1_HISTORY_STABILIZATION_ENERGY_HIGH", "LS1 全历程峰值绝对 STEN/SENE 超过 1E-2 时拒绝。")  # LS1 全历程峰值门禁。
    ...
    add_apdl(lines, f"MODOPT,LANB,{modes}", f"用 Block Lanczos 从最低频率起提取恰好 {modes} 阶；频率上下限留空，避免固定频带造成缺阶。")  # 配置无频带截断的特征值提取。
    ...
    add_reject_gate(lines, "B00_AVAILABLE", "NE", str(modes), "MODAL_RESULT_COUNT_MISMATCH", f"RSTP 实际可用结果集数不等于请求的 {modes} 阶时拒绝，禁止部分导出被标为完成。")  # 模态结果集总数硬门禁。
    ...
    add_apdl(lines, "/NOPR", "关闭命令回显，使属性文件只保留 *VWRITE 生成的纯数值记录。")  # 在属性导出前抑制批处理输入回显。
    ...
    add_apdl(lines, "/GOPR", "恢复命令回显，使后续节点导出和退出摘要保留正常审计轨迹。")  # 在纯 CSV 关闭后恢复批处理输入回显。
...
- upper_frequency_hz：频率搜索上限；值为 null 表示不设固定上限，以请求阶数严格闭合。
...
- static_solution_contract：LS1、LS2、端点/全历程能量、质量、支承和反力门槛；两步 stabilization 均为 OFF。
...
- b00_ls1_energy_history.csv：无标题六列纯数值 LS1 全历程；依次为载荷步、子步、伪时间、SENE/N·mm、STEN/N·mm、abs(STEN/SENE)。
...
def prepare_run(u01_run_name: str, modes: int) -> Path:  # 定义 B00 prepare-only 主流程，输入显式 U01 名和合法阶数，返回新 run 路径。
    ...
    manifest = {  # 构造根级 B00 机器清单。
        ...
        "upper_frequency_hz": None,  # 记录没有固定频率上限，使 Block Lanczos 以请求阶数而非频带作为闭合标准。
        ...
        "static_solution_contract": {"ls1": {"time": 1.0, "nsubst": [20, 200, 20], "autots": True, "stabilization": "OFF", "cnvg_required": 1, "energy_output": "VENG_ALL_CONVERGED_SUBSTEPS", "energy_history_file": f"solver/{LS1_HISTORY_FILE_STEM}.csv", "endpoint_abs_sten_over_sene_max": LS1_ENERGY_RATIO_LIMIT, "history_peak_abs_sten_over_sene_max": LS1_ENERGY_RATIO_LIMIT}, "ls2": {"time": 1.001, "nsubst": [1, 1, 1], "autots": False, "cutback_allowed": False, "stabilization": "OFF", "cnvg_required": 1, "endpoint_abs_sten_over_sene_max": LS2_ENERGY_RATIO_LIMIT}, "gravity_mm_s2": GRAVITY_MM_S2, "mass_expected_tonne": EXPECTED_MASS_TONNE, "mass_absolute_tolerance_tonne": MASS_ABSOLUTE_TOLERANCE_TONNE, "uz_support_count": EXPECTED_UZ_SUPPORT_COUNT, "reaction_relative_tolerance": REACTION_RELATIVE_TOLERANCE},  # 记录端点与全历程能量均 fail-closed 的完整静力契约。
        "modal_solution_contract": {"restart": "ANTYPE,,RESTART,2,,PERTURB", "perturb": "PERTURB,MODAL,AUTO,CURRENT,PARKEEP", "eigensolver": "LANB", "frequency_bounds_hz": None, "result_count_must_equal_requested": True, "lumped_mass": False, "mxpand_element_calculation": True, "node_solution_output": "NSOL,ALL", "frequency_precision": "E24.16", "modal_properties": ["FREQ", "GENM", "PFACT_X_Y_Z_ROTX_ROTY_ROTZ", "EFFM_X_Y_Z_ROTX_ROTY_ROTZ"], "prmass_command_used": False, "displacement_export": "mode_XX_all_nodes.txt PRNSOL,U,COMP", "rotation_export": "mode_XX_rotations.txt PRNSOL,ROT,COMP"},  # 记录无频带截断、严格阶数闭合的模态及导出契约。
