...
def make_single_mpc_uxyz_load_test_input(case_id: str, load_label: str, load_value: float) -> str:  # 输入平移载荷案例并返回生产拓扑单 TYPE72 装配秩微模型。
    ...
    lines = [  # 初始化生产拓扑微模型 APDL 行表。
        ...
        "CNVTOL,F,,0.005,2,1.0 ! 将 VALUE 留空，固定 0.5% 力容限、L2 范数和 1 N 最小参考值。",  # 按 CNVTOL,Lab,VALUE,TOLER,NORM,MINREF 官方字段顺序禁止自动放宽力判据。
        "CNVTOL,M,,0.005,2,1.0 ! 将 VALUE 留空，固定 0.5% 力矩容限、L2 范数和 1 N·mm 最小参考值。",  # 显式关闭转动平衡方程的自动容限调整。
