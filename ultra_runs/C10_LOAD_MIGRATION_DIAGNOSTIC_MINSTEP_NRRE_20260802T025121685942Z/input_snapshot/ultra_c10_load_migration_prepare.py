...
import argparse  # 解析互斥的 TYPE72 静力几何应力刚度排除或 LS2 自适应切回单变量诊断开关。
...
LS2_INITIAL_SUBSTEPS = 200  # 规定 LS2 初始子步数为二百、首个迁移增量为 0.5%；固定模式三参数同值，自适应模式仅保留此初值。
LS2_MAX_SUBSTEPS = 200  # LS2 最多二百个子步；与初始值相同，冻结 0.5% 增量并禁止求解器放大步长。
LS2_MIN_SUBSTEPS = 200  # 规定 LS2 最少二百个子步、任何迁移增量不得大于 0.5%；固定模式与初值同值，自适应模式仍作为最大步长边界。
...
def parse_args() -> argparse.Namespace:  # 无业务输入并返回命令行命名空间；两个诊断开关互斥且默认复现既有 0.5% 路径。
    ...
    mode_group.add_argument("--exclude-mpc-stress-stiffness", action="store_true", help="仅在静力阶段设置 TYPE72 KEYOPT(5)=1，并保持固定 0.5%% 迁移路径。")  # argparse 用双百分号渲染单个百分号，显式选择已执行的 K5 单差异模式。
    mode_group.add_argument("--adaptive-migration-cutback", action="store_true", help="保持 TYPE72 KEYOPT(5)=0，仅把 LS2 改为 200/2000/200，允许切回至 0.05%%。")  # argparse 用双百分号渲染单个百分号，显式选择 K5 证伪后的唯一数值细化诊断。
...
def transform_main(source_bytes: bytes, old_jobname: str, new_jobname: str, correction_audit: dict[str, Any], exclude_mpc_stress_stiffness: bool = False, adaptive_migration_cutback: bool = False) -> tuple[bytes, dict[str, Any]]:  # 输入父主控、作业身份、修正审计和两个互斥模式开关并返回两步位置迁移静力主控。
    ...
    candidate = base.replace_once(candidate, "! 固定拒绝原因 LS2_HOLD_NOT_CONVERGED 供外部审计按字段解析。", "! 固定拒绝原因 LS2_LOAD_POSITION_MIGRATION_NOT_CONVERGED 供外部审计按字段解析。", "LS2 失败原因说明")  # 使逐行注释与实际 /COM 拒绝代码完全一致。
    candidate = base.replace_once(candidate, "! 读取 LS2 无稳定化保持步的最后收敛结果。", "! 读取 LS2 无稳定化位置迁移的 beta=0 最后收敛结果。", "LS2 后处理语义")  # 使能量端点说明与新路径一致。
    candidate = base.replace_once(candidate, "! 读取当前结果集载荷步编号，用于排除 LS2 保持步。", "! 读取当前结果集载荷步编号，用于排除 LS2 位置迁移结果。", "LS1 历程过滤说明")  # 使结果集过滤注释不再误称 LS2 为零增量保持步。
...
def main() -> None:  # 读取单变量开关、验证冻结源并生成一个全新且未启动的恒总荷载位置迁移诊断包。
    ...
    new_jobname = f"{job_family}_{created_at.strftime('%m%dt%H%M%S%f')}_d1"  # 把微秒纳入不超过三十二字符的 MAPDL 作业名，关闭同秒并发准备产生同名文件族的风险。
    ...
    runtime_tool_dir = Path(__file__).resolve().parent  # 定位与准备器同目录的批准执行器和自适应监控器源文件。
    runtime_execute_source = runtime_tool_dir / "ultra_c10_static_execute.py"  # 定位启动前完整账本、资源与排他认领执行器源文件。
    runtime_monitor_source = runtime_tool_dir / "ultra_c10_adaptive_monitor.py"  # 定位允许 AUTOTS 二分且仅执行冻结硬停的监控器源文件。
    require(runtime_execute_source.is_file() and runtime_monitor_source.is_file(), "缺少批准的静力执行器或自适应运行监控器源文件")  # 自适应准备包必须同时冻结可实际调用的两段运行代码。
    shutil.copy2(runtime_execute_source, input_snapshot_dir / runtime_execute_source.name)  # 把执行器逐字节复制进准备账本，实际启动必须调用该快照。
    shutil.copy2(runtime_monitor_source, input_snapshot_dir / runtime_monitor_source.name)  # 把监控器逐字节复制进准备账本，实际监控必须调用该快照。
    ...
    adaptive_field_note = "`ls2_nsubst=[200,2000,200]` 表示初始和最大迁移增量均为 0.5%，失败时允许自动二分至最小 0.05%；首个 85 MN 残差、普通 ERROR、NCNV 或 divergence 提示不是控制器停机条件。控制器只在资源硬门、明确 FATAL、方程数漂移、small/zero/negative pivot 或 CNVTOL 被忽略/重置时中止；若 0.05% 最小增量仍不收敛，则让 MAPDL 原生退出并由终结器判定失败。" if adaptive_migration_cutback else "`ls2_nsubst=[200,200,200]` 表示固定 0.5% 且不允许 cutback。"  # 解释自适应步长上下界、普通 NCNV 非硬停和控制器不得提前停止的行为合同。
