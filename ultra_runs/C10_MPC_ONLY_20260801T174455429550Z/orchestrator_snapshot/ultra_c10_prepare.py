...
RIGID_TYPE_ID = 72  # MPC184 rigid-beam 的空闲 TYPE 编号，用于 ALL 刚接及 UXYZ 偏置刚臂。
TRANSLATION_LINK_TYPE_ID = 73  # MPC184 displacement-only rigid-link 的空闲 TYPE 编号，用于 UXYZ 共点三平移精确约束。
...
def make_definition_block(newline: str) -> str:  # 输入源换行并返回可逆 MPC184 定义块。
    """定义 TYPE72 rigid beam 与 TYPE73 displacement-only rigid link。"""  # 返回文本每条 APDL 命令均有中文注释。
    lines = [DEFINITION_BEGIN, f"ET,{RIGID_TYPE_ID},MPC184 ! TYPE72 定义零质量六自由度大转动刚臂", f"KEYOPT,{RIGID_TYPE_ID},1,1 ! 选择 MPC184 rigid-beam 公式", f"KEYOPT,{RIGID_TYPE_ID},2,0 ! 使用直接消元精确实施刚臂约束", f"KEYOPT,{RIGID_TYPE_ID},5,0 ! 保留 rigid-beam 的几何刚度以支持预应力模态", f"ET,{TRANSLATION_LINK_TYPE_ID},MPC184 ! TYPE73 定义 UXYZ 共点三平移 rigid link", f"KEYOPT,{TRANSLATION_LINK_TYPE_ID},1,0 ! 选择只激活 UX、UY、UZ 的 rigid-link 公式", f"KEYOPT,{TRANSLATION_LINK_TYPE_ID},2,0 ! 使用直接消元精确实施共点三平移约束", "CSYS,0 ! 恢复全局笛卡尔坐标，保证后续原节点坐标语义不变", DEFINITION_END]  # 完整定义块按稳定顺序组装且不再引入罚因子。
...
def make_connection_block(newline: str, records: list[dict[str, Any]], nodes: dict[int, dict[str, Any]]) -> tuple[str, list[dict[str, Any]]]:  # 输入连接和全模型节点并返回 APDL 替换块及映射台账。
    """ALL 生成一个 TYPE72；UXYZ 生成共点辅助节点、一个 TYPE72 和一个 TYPE73 rigid link。"""  # 编号严格使用两个封板连续区间。
    ...
    for index, record in enumerate(records):  # 按父 CERIG 原顺序生成全部 MPC184。
        ...
        link_element: int | None = None  # ALL 无 TYPE73，UXYZ 后续填入第二个三平移刚性链元素号。
        ...
        if record["dof_label"] == "ALL":  # 六自由度刚接直接连接原 master 和 slave。
        ...
        else:  # UXYZ 采用偏置刚臂和共点三平移 rigid link 的复合实现。
            ...
            link_element = next_element  # 保存复合实现第二个 TYPE73 rigid-link 元素号。
            lines.append(f"TYPE,{TRANSLATION_LINK_TYPE_ID} ! 选择 TYPE73 rigid link 实现 {record['conn_id']} 的三平移关系")  # 切换为 displacement-only rigid-link 类型。
            lines.append(f"EN,{next_element},{aux_node},{slave} ! {record['conn_id']}：辅助节点到 slave 的共点三平移直接消元连接；装配={assembly}")  # 写复合实现第二元素。
            next_element += 1  # rigid link 消耗第二个元素号。
            implementation = "MPC184_TYPE72_OFFSET_RIGID_BEAM_PLUS_TYPE73_COINCIDENT_TRANSLATION_RIGID_LINK"  # 记录 UXYZ 无罚函数复合实现说明。
        mapping_rows.append({"conn_id": record["conn_id"], "source_file": PARENT_INCLUDE_NAME, "source_line": record["source_line"], "original_command": record["original_command"], "master_node": master, "slave_node": slave, "dof_label": record["dof_label"], "system": record["system"], "assembly_name": record["assembly_name"], "reason": record["reason"], "aux_node": "" if aux_node is None else aux_node, "rigid_element": rigid_element, "translation_link_element": "" if link_element is None else link_element, "implementation": implementation})  # 保存完整六字段源证和候选实体映射且不再记录不存在的罚因子。
...
def transform_include(source_bytes: bytes, records: list[dict[str, Any]], nodes: dict[int, dict[str, Any]]) -> tuple[bytes, list[dict[str, Any]], dict[str, Any]]:  # 输入父 include 字节并返回候选、映射和可逆审计。
    ...
    definition_block = make_definition_block(newline)  # 生成 rigid-beam 与 displacement-only rigid-link 定义块。
    connection_block, mapping_rows = make_connection_block(newline, records, nodes)  # 生成辅助节点和 MPC184 替换块。
...
def transform_main(source_bytes: bytes, jobname: str) -> tuple[bytes, dict[str, Any]]:  # 输入最终 S10 主控和新 jobname 并返回可逆 C10 主控及审计。
    ...
    query_lines = [MAIN_QUERY_BEGIN, "! 只选择 TYPE72 rigid-beam 元素，用于执行期连接数量门禁。", f"ESEL,S,TYPE,,{RIGID_TYPE_ID}", "! 读取 TYPE72 数量，预期 5078。", "*GET,C10_T72,ELEM,0,COUNT", "! 只选择 TYPE73 displacement-only rigid-link 元素，用于执行期连接数量门禁。", f"ESEL,S,TYPE,,{TRANSLATION_LINK_TYPE_ID}", "! 读取 TYPE73 数量，预期 3124。", "*GET,C10_T73,ELEM,0,COUNT", MAIN_QUERY_END]  # 构造类型查询块，每条命令均有紧邻中文说明。
    ...
    output_lines = [MAIN_OUTPUT_BEGIN, "! 第三行写 TYPE72 刚臂和 TYPE73 三平移 rigid-link 计数。", "! 下一条 *VWRITE 与其后裸 Fortran 格式行必须相邻，二者之间不得插入注释。", "*VWRITE,C10_T72,C10_T73", "('TYPE72=',F12.0,', TYPE73=',F12.0)", MAIN_OUTPUT_END]  # 构造合法 *VWRITE 输出块。
...
def graph_audit(paths: tuple[Path, ...], nodes: dict[int, dict[str, Any]], mapping_rows: list[dict[str, Any]]) -> dict[str, Any]:  # 输入父依赖、节点表和候选映射并返回静态连通审计。
    ...
    for row in mapping_rows:  # 把候选 MPC184 边加入图并检查辅助节点。
        ...
        if aux_value == "":  # ALL 连接只有直接 master—slave 边。
            ...
            graph.union(aux_node, slave)  # 加入 TYPE73 共点三平移 rigid-link 边。
...
def build_connection_ir(records: list[dict[str, Any]], mapping_rows: list[dict[str, Any]], nodes: dict[int, dict[str, Any]]) -> dict[str, Any]:  # 输入连接与映射并返回正式连接 IR。
    ...
    for record in records:  # 按父 CERIG 顺序保持稳定输出。
        ...
        dof_behavior = {"UX": "RIGID_DIRECT_ELIMINATION", "UY": "RIGID_DIRECT_ELIMINATION", "UZ": "RIGID_DIRECT_ELIMINATION", "RX": "DOF_NOT_COUPLED_OR_SLAVE_DOF_NOT_PRESENT" if is_uxyz else "RIGID_DIRECT_ELIMINATION", "RY": "DOF_NOT_COUPLED_OR_SLAVE_DOF_NOT_PRESENT" if is_uxyz else "RIGID_DIRECT_ELIMINATION", "RZ": "DOF_NOT_COUPLED_OR_SLAVE_DOF_NOT_PRESENT" if is_uxyz else "RIGID_DIRECT_ELIMINATION"}  # 明确六个自由度的相对行为且三项平移均为精确直接消元。
        connections.append({"conn_id": conn_id, "connection_class": "RIGID_OFFSET_PLUS_TRANSLATION_RIGID_LINK" if is_uxyz else "SIX_DOF_RIGID_CONNECTION", "master_node": record["master_node"], "slave_node": record["slave_node"], "aux_node": None if mapping["aux_node"] == "" else int(mapping["aux_node"]), "master_xyz_mm": list(nodes[int(record["master_node"])]["xyz"]), "slave_xyz_mm": list(nodes[int(record["slave_node"])]["xyz"]), "dof_behavior": dof_behavior, "stage": "ACTIVE_FOR_ALL_S10_INHERITED_STAGES", "reference_coordinate_system": "GLOBAL_X_LONG_Y_TRANSVERSE_Z_VERTICAL", "solver_mapping": {"rigid_type": RIGID_TYPE_ID, "rigid_element": mapping["rigid_element"], "translation_link_type": TRANSLATION_LINK_TYPE_ID if is_uxyz else None, "translation_link_element": None if mapping["translation_link_element"] == "" else int(mapping["translation_link_element"]), "constraint_imposition": "DIRECT_ELIMINATION", "penalty_n_per_mm": None}, "source_ref": {"file": PARENT_INCLUDE_NAME, "line": record["source_line"], "command": record["original_command"], "metadata_file": str(CONSTRAINT_METADATA_PATH), "system": record["system"], "assembly": record["assembly_name"], "reason": record["reason"]}, "assumption_ref": "C10-ASSUMPTION-UXYZ-001" if is_uxyz else "C10-ASSUMPTION-ALL-001", "drawing_ref": None})  # 保存一条完整可追溯连接 IR，并显式声明不存在罚刚度。
...
def build_symbolic_constraint_matrix(mapping_rows: list[dict[str, Any]]) -> dict[str, Any]:  # 输入候选映射并返回有限转动符号约束行表。
    """TYPE72 记录六个非线性刚体运动学行，TYPE73 记录三个共点平移直接消元行。"""  # 不伪造数值切线矩阵秩。
    ...
    for mapping in mapping_rows:  # 按稳定连接顺序展开求解器无关约束语义。
        ...
        if aux is not None:  # UXYZ 还需要三条共点平移直接消元约束。
            for dof in ("UX", "UY", "UZ"):  # 仅展开三个平移自由度。
                row_key = ("COINCIDENT_TRANSLATION_DIRECT_ELIMINATION", aux, slave, dof)  # 构造 rigid-link 重复检查键。
                if row_key in row_keys:  # 完全相同端点和自由度表示重复。
                    duplicate_count += 1  # 累加重复数量。
                row_keys.add(row_key)  # 保存当前符号行键。
                rows.append({"row_id": f"{conn_id}-L-{dof}", "conn_id": conn_id, "formulation": "COINCIDENT_TRANSLATION_DIRECT_ELIMINATION", "dof": dof, "node_i": aux, "node_j": slave, "coefficients": [{"node": aux, "dof": dof, "value": -1.0}, {"node": slave, "dof": dof, "value": 1.0}]})  # 保存常系数相对平移直接消元行。
    ...
    return {"schema_version": 1, "status": "SYMBOLIC_ROWS_COMPLETE_NUMERICAL_RANK_PENDING", "row_count": len(rows), "duplicate_structural_row_count": duplicate_count, "nonlinear_rigid_body_row_count": EXPECTED_LOGICAL_CONNECTIONS * 6, "linear_direct_elimination_row_count": EXPECTED_UXYZ_CONNECTIONS * 3, "numeric_rank_evaluated": False, "numeric_rank_reason": "MPC184 finite-rotation tangent depends on assembled state and must be checked after solver assembly", "rows": rows}  # 返回符号行表并明确数值秩边界。


def make_uxyz_load_test_input(case_id: str, load_label: str, load_value: float) -> str:  # 输入 UXYZ 案例身份和单位荷载并返回未执行 APDL 小模型。
    """验证偏置刚臂加共点 rigid link 的六向载荷传递和三转角释放。"""  # 返回 LF 文本且每条 APDL 命令均有中文说明。
...
def make_finite_rotation_test_input(axis_name: str, rotation_label: str, rotation_value_rad: float) -> str:  # 输入转轴、APDL 标签和规定转角并返回有限转动测试。
    """以非共线偏置向量验证 TYPE72 加 TYPE73 的 Rodrigues 刚体运动。"""  # 三个轴均使用 0.1 rad 且输出辅助与 slave 位移。
...
def build_unit_test_plan(unit_test_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:  # 输入目标目录并生成 15 份未执行微模型和计划/结果摘要。
    """六个 UXYZ 荷载、三个有限转动和六个 ALL 荷载共同构成硬门禁。"""  # 返回计划和未执行结果对象。
    cases: list[dict[str, Any]] = []  # 初始化十五个案例计划。
    for load_name, load_label, load_value, load_unit in UNIT_LOAD_CASES:  # 生成六个 UXYZ 单位荷载案例。
        case_id = f"UXYZ-{load_name}"  # 形成稳定 UXYZ 案例身份。
        file_name = f"c10_uxyz_{load_name.lower()}.inp"  # 形成安全 ASCII 输入文件名。
        input_text = make_uxyz_load_test_input(load_name, load_label, load_value)  # 生成当前 UXYZ APDL 模板。
        write_new_text(unit_test_dir / file_name, input_text)  # 以不可覆盖模式写当前案例。
        cases.append({"case_id": case_id, "case_family": "UXYZ_SIX_AXIS_LOAD_TRANSFER", "load_label": load_label, "load_value": load_value, "load_unit": load_unit, "input_file": f"unit_tests/{file_name}", "input_sha256": sha256_bytes(input_text.encode("utf-8")), "future_result_file": f"unit_tests/c10ux_{load_name.lower()}_results.txt", "status": "PREPARED_NOT_STARTED"})  # 保存 UXYZ 荷载案例合同。
    for axis_name, rotation_label, rotation_value_rad in FINITE_ROTATION_CASES:  # 生成三个 0.1 rad 有限转动案例。
        ...
        input_text = make_finite_rotation_test_input(axis_name, rotation_label, rotation_value_rad)  # 生成当前有限转动 APDL 模板。
        write_new_text(unit_test_dir / file_name, input_text)  # 以不可覆盖模式写当前案例。
        cases.append({"case_id": case_id, "case_family": "UXYZ_FINITE_ROTATION_KINEMATICS", "rotation_label": rotation_label, "rotation_value_rad": rotation_value_rad, "offset_vector_mm": [800.0, -600.0, 500.0], "input_file": f"unit_tests/{file_name}", "input_sha256": sha256_bytes(input_text.encode("utf-8")), "future_result_file": f"unit_tests/c10fr_{axis_name.lower()}_results.txt", "status": "PREPARED_NOT_STARTED"})  # 保存 Rodrigues 解析验收所需几何和转角。
    ...
    require(len(cases) == 15, f"单位测试案例数为 {len(cases)}，预期 15")  # 六加三加六数量必须闭合。
    plan = {"schema_version": 2, "status": "PREPARED_NOT_STARTED", "purpose": "C10_DIRECT_ELIMINATION_KINEMATICS_AND_SIX_AXIS_TRANSFER", "planned_case_count": len(cases), "completed_case_count": 0, "mapdl_execution_attempted": False, "formulation": {"UXYZ": "TYPE72_OFFSET_RIGID_BEAM_PLUS_TYPE73_COINCIDENT_TRANSLATION_RIGID_LINK", "ALL": "TYPE72_RIGID_BEAM", "constraint_imposition": "DIRECT_ELIMINATION", "penalty_factor": None}, "acceptance": {"max_relative_translation_slip_mm": 1.0e-9, "finite_rotation_rodrigues_displacement_error_mm": 1.0e-6, "force_equilibrium_absolute_tolerance_n": 1.0e-8, "moment_equilibrium_absolute_tolerance_n_mm": 1.0e-6, "all_slave_translation_and_rotation_max_abs": 1.0e-10, "automatic_cnvtol_reset_allowed": False, "zero_or_negative_pivot_allowed": False}, "cases": cases}  # 汇总十五个无罚函数硬门禁案例及数值阈值。
    plan["result_contract"] = {"reported_vectors": ["AUX_U_XYZ_MM", "SLAVE_U_XYZ_MM", "SLAVE_ROT_XYZ_RAD", "MASTER_REACTION_FORCE_XYZ_N", "MASTER_REACTION_MOMENT_XYZ_N_MM", "GROUND_REACTION_FORCE_XYZ_N", "GROUND_REACTION_MOMENT_XYZ_N_MM", "MASTER_ROTATION_XYZ_RAD"], "derived_checks": ["UXYZ_ZERO_TRANSLATION_SLIP", "UXYZ_RELEASED_MOMENT_PATH", "RODRIGUES_FINITE_ROTATION", "ALL_ZERO_RELATIVE_SIX_DOF", "GLOBAL_FORCE_EQUILIBRIUM", "GLOBAL_MOMENT_EQUILIBRIUM_ABOUT_MASTER", "NO_AUTOMATIC_CONVERGENCE_RELAXATION", "NO_ZERO_OR_NEGATIVE_PIVOT"], "sufficient_for_declared_micro_acceptance": True}  # 明确结果文件与日志共同覆盖全部微模型门禁。
    results = {"schema_version": 2, "status": "NOT_RUN", "planned_case_count": 15, "completed_case_count": 0, "passed_case_count": 0, "failed_case_count": 0, "mapdl_execution_attempted": False, "result_files_present": 0, "numerical_claim_allowed": False}  # 明确 prepare 阶段没有任何数值通过结论。
...
def prepare(parent_run: str, run_name: str) -> Path:  # 输入父身份和目标身份并返回生成运行目录。
    ...
    require(len({int(row["rigid_element"]) for row in mapping_rows} | {int(row["translation_link_element"]) for row in mapping_rows if row["translation_link_element"] != ""}) == EXPECTED_MPC_ELEMENTS, "MPC184 元素 ID 不唯一")  # 候选元素号必须完整唯一。
    ...
    unit_test_dir = run_dir / "unit_tests"  # 十五个未执行硬门禁微模型输入目录。
    ...
    unit_test_plan, unit_test_results = build_unit_test_plan(unit_test_dir)  # 生成 15 个未执行硬门禁输入及计划/结果状态。
    ...
    constraint_audit = {"schema_version": 2, "status": "STATIC_MAPPING_PASSED_NUMERICAL_RANK_PENDING", "logical_connections": EXPECTED_LOGICAL_CONNECTIONS, "source_semantics": {"UXYZ": EXPECTED_UXYZ_CONNECTIONS, "ALL": EXPECTED_ALL_CONNECTIONS}, "candidate_elements": {"TYPE72_RIGID_BEAM": EXPECTED_LOGICAL_CONNECTIONS, "TYPE73_TRANSLATION_RIGID_LINK": EXPECTED_UXYZ_CONNECTIONS, "total": EXPECTED_MPC_ELEMENTS}, "constraint_imposition": {"TYPE72": "DIRECT_ELIMINATION", "TYPE73": "DIRECT_ELIMINATION", "penalty_factor": None}, "id_checks": {"aux_ids_unique": True, "aux_range": [AUX_NODE_START, AUX_NODE_END], "mpc_ids_unique": True, "mpc_range": [MPC_ELEMENT_START, MPC_ELEMENT_END], "node_collision_count": 0, "explicit_element_collision_count": 0, "existing_explicit_element_id_count": len(existing_element_ids)}, "reference_checks": {"all_master_slave_nodes_exist": True, "uxyz_slave_unique_count": len({int(row["slave_node"]) for row in mapping_rows if row["dof_label"] == "UXYZ"}), "master_equals_slave_count": 0, "duplicate_directed_semantic_pair_count": 0}, "coincidence_checks": {"authorized_zero_length_type73_rigid_link_count": EXPECTED_UXYZ_CONNECTIONS, "aux_coordinate_strings_copied_from_slave": True, "unexpected_zero_length_physical_beam_count": 0}, "legacy_constraint_commands": {"parent_cerig": EXPECTED_LOGICAL_CONNECTIONS, "candidate_cerig": 0, "cp_unchanged": parent_counts["CP"], "d_unchanged": parent_counts["D"]}, "symbolic_matrix": {"row_count": symbolic_matrix["row_count"], "duplicate_structural_row_count": symbolic_matrix["duplicate_structural_row_count"], "numeric_rank_evaluated": False}, "graph_audit": graph}  # 汇总全直接消元静态约束审计和待求解边界。
    ...
    support_expectation = {"schema_version": 1, "status": "NOT_EVALUATED_PREPARE_ONLY", "reference_coordinate_system": "GLOBAL_X_LONG_Y_TRANSVERSE_Z_VERTICAL", "future_checks": [{"id": "REACTION-FORCE-01", "rule": "sum_support_reactions_plus_applied_force_equals_zero", "relative_tolerance": 1.0e-6}, {"id": "REACTION-MOMENT-01", "rule": "sum_support_moments_plus_force_moments_about_frozen_origin_equals_zero", "relative_tolerance": 1.0e-6}, {"id": "INTERFACE-FLOW-01", "rule": "unit_test_master_and_ground_reactions_close_applied_unit_load", "relative_tolerance": 1.0e-6}], "numeric_expected_values_available": False, "reason": "full-bridge solve and 15 direct-elimination micro solves were intentionally not started"}  # 定义未来反力平衡规则而不伪造数值。
    write_new_json(qa_dir / "support_reaction_expectation.json", support_expectation)  # 写支承反力预期。
    connection_issues = {"schema_version": 1, "status": "OPEN_ITEMS_RESTRICT_PRODUCTION_USE", "issues": [{"id": "C10-DWG-01", "severity": "MEDIUM", "status": "OPEN", "description": "5078 条连接有 S10/CERIG 和 builder 元数据来源，但没有逐条图纸编号引用。", "effect": "candidate may be used for connection-repair validation only"}, {"id": "C10-NUM-01", "severity": "HIGH", "status": "OPEN", "description": "十五个全直接消元微算例、全桥静力、装配秩和模态响应尚未执行。", "effect": "launch and production claims remain disabled"}, {"id": "C10-RANK-01", "severity": "MEDIUM", "status": "OPEN", "description": "有限转动 MPC184 直接消元链的装配切线数值秩必须在 MAPDL 组装后验证。", "effect": "static symbolic audit is not a numerical rank proof"}, {"id": "C10-BUILDER-01", "severity": "LOW", "status": "DOCUMENTED", "description": "旧物理 builder 所需 tail_nodes.csv 已缺失，且旧重生方法会污染最终 S10 编号边界。", "effect": "this run uses an exact reversible patch instead"}]}  # 记录未关闭问题和使用限制。
    write_new_json(qa_dir / "connection_issues.json", connection_issues)  # 写连接问题清单。
    model_single_difference = {"schema_version": 2, "status": "PASSED", "parent_run": parent_run, "physical_change_family_count": 1, "physical_change_family": "CERIG_TO_MPC184_LARGE_ROTATION_CONNECTION_KINEMATICS_ONLY", "invariant_dependency_count": len(INVARIANT_DEPENDENCY_NAMES), "controlled_dependency_count": 1, "include_reversible_audit": include_audit, "main_control_reversible_audit": main_audit, "allowed_changes": {"cerig_removed": EXPECTED_LOGICAL_CONNECTIONS, "aux_nodes_added": EXPECTED_UXYZ_CONNECTIONS, "type72_rigid_beams_added": EXPECTED_LOGICAL_CONNECTIONS, "type73_translation_rigid_links_added": EXPECTED_UXYZ_CONNECTIONS, "mpc_type_and_component_definitions_added": True}, "forbidden_changes": {"original_29626_node_commands": False, "original_17679_beam_commands": False, "materials": False, "sections_61_to_66": False, "beam_orientation_nodes": False, "mass": False, "loads": False, "initial_state": False, "boundary_d": False, "boundary_cp": False, "analysis_stages": False, "modal_request": False, "penalty_stiffness": False}, "parent_section_61_exact_line_preserved": "SECDATA,4997.5,9830899.73958,0,28164706.4583,66066802083.3,176798.958333,0,0,0,0,175,175,0.6712958043168660,0.2363842696133852" in candidate_include_bytes.decode("utf-8")}  # 汇总单物理差异合同和无罚函数禁改项。
