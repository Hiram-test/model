...
class SourceSpreader:  # Represent a source-located deformation frame as a finite-rotation rigid spreader, never as a grounded rotation or a thin floor crossbeam.
    def __init__(self,deck,x,z,ys,profile,owner,tag):  # Preserve the source support location and all separate physical rope ports.
        ...
        origin=deck.nodes[self.ref];self.members=[deck.newnode(origin+np.array([.31,0.,0.])),deck.newnode(origin+np.array([0.,.37,0.])),deck.newnode(origin+np.array([0.,0.,.41]))];deck.bodies.append((self.members,self.ref,self.rot,tag+'_source_spreader'));deck.owners[owner]+=0.  # Orientation witnesses have no stiffness or mass; original frame mass is assigned separately from its source load.
