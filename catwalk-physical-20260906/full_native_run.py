...
def compute(source,state):  # Execute the complete source-based native static-plus-modal model.
    ...
    model.make_members();model.make_ropes_and_mass();model.save(folder);model.d.write(folder/'bridge.inp',modes=20,gravity=source['g'])  # Request the first twenty native roots in solver order for the fourteen-mode task; this only reduces modal-vector memory and does not alter stiffness, mass, or preprocessing requirements.
