...
HERE = Path(__file__).resolve().parent  # Locate the inverse-form-finding directory. Residual 0.00328 is not verified INIFORCE.
...
sys.path.insert(0, str(HERE))  # Make the inverse solver modules importable. Residual 0.00328 is not verified INIFORCE.
sys.path.insert(0, str(CLEAN_DIR))  # Make the existing dynamic solver modules importable.
import solve_modal_from_inverse as previous  # type: ignore  # Reuse recover_static_state. Residual 0.00328 is not verified INIFORCE.
...
def main() -> int:  # Execute the complete no-transverse-passage dynamic ablation.
    ...
    static = previous.recover_static_state(model)  # Recompute the full-system MCT-geometry prestress state before dynamic ablation. Residual 0.00328 is not verified INIFORCE.
    ...
    if float(static["max_abs_force_error_percent"]) > 0.5:  # Preserve the declared elementwise initial-force agreement threshold.
        raise RuntimeError(f"Maximum MCT initial-force mismatch is {static['max_abs_force_error_percent']:.6f}%")  # Reject a prestress state that no longer matches the stored MCT force field within 0.5%. The 0.5% gate is not recovered INIFORCE.
