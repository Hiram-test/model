...
def main() -> int:  # Execute the comparison only after the target-free frozen result exists.
    ...
    for label in LABEL_ORDER:  # Traverse every requested physical family.
        ...
        rows.append({"label": label, "mode": record.get("mode"), "computed_hz": computed, "target_hz": target, "error_percent": error, "status": record.get("status", "identified"), "harmonic_order_descriptor": record.get("harmonic_order_descriptor"), "floor_gantry_acoustic_coherence": record.get("floor_gantry_acoustic_coherence"), "selection_rule": record.get("selection_rule")})  # Store the complete post-freeze comparison row.
