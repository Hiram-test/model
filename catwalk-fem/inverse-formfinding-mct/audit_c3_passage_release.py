...
def main() -> int:  # Download, parse, group, and audit the C3 passage topology.
    ...
    passage_candidates = [item for item in set_summaries if item["node_count"] >= 100 and max(item["extent"]) >= 30000.0 and min(item["extent"]) <= 30000.0]  # Select large transverse local substructures by size and coordinate extent only.
    passage_candidates.sort(key=lambda item: (abs(item["node_count"] - 339), abs(item["element_count"] - 639), item["name"]))  # Rank candidates by the independently audited passage counts.
    ...
    report = {"source_url": URL, "deck_bytes": DECK.stat().st_size, "node_count": len(nodes), "element_count": len(elements), "elset_count": len(elset_members), "nset_count": len(nset_members), "passage_candidates": passage_candidates[:80], "semantically_named_candidates": names_of_interest[:80], "largest_element_sets": sorted(set_summaries, key=lambda item: item["element_count"], reverse=True)[:80], "element_block_counts": {name: dict(counts) for name, counts in element_block_summary.items()}}  # Assemble the complete topology-audit report.
