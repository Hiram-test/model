...
门架计数单位：**榀（U+6980）**。不是榌（U+698C）。上一轮把榌写成榀的变体，本轮更正：榌不得当对账单位。
...
| 142 榀门架对账（不是榌） | 71×2=142，单位 U+6980，禁止 U+698C | 用户更正 |
...
| 142 榀门架（不是榌） | **成立** | 71×2=142，单位 U+6980，禁止 U+698C |
...
**总评：新主 deck 过门（§7.76 PK2 + 新哈希 + 冻结 82548e6a 未改 + 21/142 榀（不是榌）+ 分锚）。CalculiX 2.21 读入过门。求解切线因 22 096 个不连通分量奇异，不是已求解过门，不是模态过门。论文已产出。**
