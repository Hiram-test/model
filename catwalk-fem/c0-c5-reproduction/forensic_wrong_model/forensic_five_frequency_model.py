...
def main() -> None:  # 执行严格标签与最可能分支重识别两条路径。
    ...
    record["status"] = "RETAINED_INVERSE_REACHABILITY"  # 保留为逆向可达性档案，不是附件复现。
    record["frequency_reproduced"] = False  # 禁止把目标条件拟合写成频率已复现。
    record["causal_conclusion"] = False  # 禁止把反求系数写成对方 APDL 因果结论。
    record["note"] = "target-conditioned inverse reachability; TA1 0.099600 +0.00% is a fit residual, not attach reproduction"  # 锁定 +0.00% 的非复现读法。
