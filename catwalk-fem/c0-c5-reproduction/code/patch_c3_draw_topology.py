#!/usr/bin/env python3
"""Drawing passage pin: drop pure ROT weld only. Keep u=u+theta x r."""
...
def classify_equation(terms, coords_y, node_fam):
    ...
    if is_bottom_hoop:
        return "KEEP"
    if has_passage and has_rot and not has_trans:
        return "DROP_PURE_ROT"
    return "KEEP"
    ...
    with src.open(encoding="utf-8") as handle, dst.open("w", encoding="utf-8", newline="\n") as out:
        for raw in handle:
            ...
            if state == "eqt":
                ...
                elif action == "DROP_PURE_ROT":
                    stats["drop_pure_rot"] += 1
                else:
                    write_equation(out, terms)
                    stats["keep"] += 1
                    if has_trans and has_rot:
                        stats["keep_mixed_lever"] += 1
                state = "eqc"
                continue
            out.write(raw)
    return stats
...
def main():
    ...
    if stats["drop_pure_rot"] < 1:
        raise SystemExit("no pure-ROT passage weld dropped")
    if stats["keep_mixed_lever"] < 1:
        raise SystemExit("lever translation equations were not kept")
    dst_sha = sha256_file(dst)
    receipt = {
        "schema": "catwalk.c3-ub-ft14.draw-topology.v6-pin-keep-lever",
        "source": {"path": src.name, "sha256": src_sha, "bytes": src.stat().st_size},
        "output": {"path": dst.name, "sha256": dst_sha, "bytes": dst.stat().st_size},
        "rule": {
            "passage": "drop pure ROT weld only; keep full u=u+theta*r translation",
            "saddle": "unchanged",
            "equalizer": "unchanged",
            "friction": False,
            "contact": False,
