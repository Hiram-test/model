...
LABELS = {1: "LS1", 2: "VA1", 3: "TA1", 4: "LA1", 5: "VS1", 6: "TS1", 7: "LS2", 8: "SIDE1", 9: "SIDE2", 11: "VA2", 12: "LA2", 13: "TS2", 15: "SIDE3", 21: "VS2"}  # Freeze previously verified C3 physical branch labels.
FORENSIC_LABELS = ("TA1", "TS1", "TS2", "SIDE3", "VS2")  # Select the five branches under historical-workflow study.
...
def main() -> None:  # Execute global C3-to-Track-A mode tracking.
    parser = argparse.ArgumentParser(description="Track the strict old-ANSYS Track A forward model against the unchanged C3 baseline.")  # Define the command-line interface.
    ...
    lines = ["# C3 至旧版 ANSYS 式 Track A 的全局一对一模态追踪", "", "分支配对不读取附件目标频率，也不采用逐项最近频率。C3 前 40 阶通过矩形 MAC 矩阵唯一分配到 Track A 前 80 阶；左右幅相位、滚转参与率和纵向符号变化只用于解释旧版 ANSYS 人工命名。", "", "| 分支 | C3阶次/Hz | Track A阶次/Hz | 频率变化 | Δω² | MAC | Track A前14阶内 | 左右幅相关 | 滚转参与率 | 纵向符号变化 |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]  # Initialize the compact human-readable report.
    ...
    lines.extend(["", "## Track A 自然前14阶", "", "| 阶次 | 频率/Hz |", "|---:|---:|"])  # Add the natural solver-order table header.
