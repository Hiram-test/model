...
N=40 正弦 Galerkin，v3 §3.2 超越式与满阵特征值各算一遍。常数全部从 v3 公式当场生成：F_M=255.56（锁定）、H_0=μ_M g L_M²/(8 F_M)=8.39345 MN、L_e=2375.7228 m、(EA)_upper=16 E_s (12.038/7850)=5.05443 GN。**不用 227.30。**

偶数 2f_01 = **0.069258 Hz，与 η 无关**。奇数张弦极点（0.034629、0.103887）是分母零点，不是根。

对本稿 §3.3 η 表（本轮重算与表差 < 1 µHz）：

| η | v3 根2（第一移动根） | 本轮超越式 / 满阵特征值 |
|---|---|---|
| 0.1 | 0.072902 | 0.072902 |
| 0.3 | 0.093779 | 0.093779 |
| 0.5 | 0.096615 | 0.096615 |
| 0.7 | 0.097491 | 0.097491 |
| 1.0 | 0.098043 | 0.098043 |
...
| 0.40 | 0.095693 |
| **0.53** | **0.096802** |
| 0.5294（复核报告 EA） | 0.096799 |

η=0.53 时超越式残差 4.6×10⁻¹⁵。0.096802 落在 v3 已印的 0.5–0.7 之间（0.096615 → 0.097491），线性插值约 0.096746，本轮根比插值高 56 µHz，是曲线在高 η 变平，不是算错。

### 8.3a 第 6 根复查（F_att = 0.0733 Hz）

目标锁在第 6 个 F_att：**0.0733 Hz**。中档把这一行对到「M2 第一移动根、η=0.53」，写成 0.096802（+32.1%）。下面只回答：是乘错、对槽对错，还是图纸 η 把这根抬走。

**不是乘错，也不是公式抄错。**

- 相对误差就是 (0.096802 − 0.0733) / 0.0733 = **+32.06%**。分子分母没有写反，也没有把 Hz 和 rad/s 混用。
- 同一套 v3 §3.2 公式复现了 §3.3 全表；η=0.53 只是表上没有的中间点。把 0.096802 改成 0.0733 才是算错。
- 若误用禁垂度 227.30 m，偶数根 2f_01 会变成 0.07344 Hz，看起来像打中第 6 根。本轮锁定 F_M=255.56，偶数根仍是 0.069258（第 5 行），没有走这条捷径。

**对槽：M12 写的 0.0729 是 η=0.1 的扫描点，不是图纸 η。**

v3 §13.3（M12 分支计数）原文把第 5–7 阶里的 M2 移动根写成

```
M2在较低有效轴伸刚度下的第一移动根  0.0729 Hz
```

0.0729 Hz 就是 §3.3 表 **η=0.1 的根2 = 0.072902 Hz**（对 0.0733 只有 −0.54%）。v3 自己也写了「η 只是独立参数扫描，不由附件频率确定」。一稿把 0.0733 对到 0.0729，等于暗中取了扫描档 η=0.1。

**+32% 的物理来源是图纸 η 把同一根抬走。**

秩一轴伸把奇数 n=1 张弦根（0.034629）向上推，越过偶数 2f_01 之后继续升，高 η 时逼近不可伸长上限（约 2.86 f_01 ≈ 0.099 Hz）。η=0.53 已经靠近这条天花板：

```
η = 0.10   →  0.072902 Hz     （M12 引用的「较低轴伸」点）
η = 0.53   →  0.096802 Hz     （复核报告 EA / v3 上界，本行）
η = 1.00   →  0.098043 Hz
```

中档 η=0.53 时，M2 在 0.073 附近**没有第二根**：0.069258 已分给第 5 行（偶数、与 η 无关）；下一根就是 0.096802。要把移动根按回 0.0733，超越式反解给出 η≈0.102（EA≈514 MN）。那是 **F_att 反标**，本文不用。

结论：第 6 行的 +32% **应保留**。它不是计算事故，是「一稿按 η=0.1 对槽」和「图纸 η=0.53 把移动根抬到 0.097」叠在一起。第 5 行偶数根仍然成立。第 6 行在当前图纸深度下没有第二个 M2 单点。
...
第 6 行复查见 §8.3a：0.096802 与 v3 η 表一致，不是乘错；M12 的 0.0729 是 η=0.1 扫描档对槽；图纸 η=0.53 把同一根抬到 0.097。要按回 0.0733 只能反标 η≈0.10，本文拒绝。0.0733 / 0.0735 在当前图纸深度下没有第二个 M2 单点。
