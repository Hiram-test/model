...
def build_toolchain(args: argparse.Namespace) -> int:  # Capture exact runner, Python, CalculiX, and linked-library identities for provenance.
    ...
    version_identity_pass = version_result.returncode in (0, 201) and re.fullmatch(r"\s*This is Version 2\.17\s*", version_result.stdout) is not None and not version_result.stderr  # Accept CalculiX's documented information exit 201 only with the exact pinned 2.17 identity text and empty error stream.
