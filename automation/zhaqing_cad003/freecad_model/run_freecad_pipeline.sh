...
# 锚碇/风缆复核、吊杆索夹间隙、Compound STEP 封装、原生 FCStd 校验、STEP 回读、
# 禁止穿透审计、专项几何审计或渲染失败，最终退出码均非零；上层 Actions 会先
# 上传已有日志和中间文件，再执行最终失败判定。
set -euo pipefail
...
# FreeCAD 0.19 有时会在 Python 异常后仍返回 0；所以每个阶段除退出码外，
# 还必须产生带确定状态字段的 JSON receipt。
assert_json_equals() {
  local json_path="$1"
...
# Step 0: 在任何可失败动作前记录运行入口、工具版本，并编译实际执行的全部脚本。
{
  ...
  python3 -m py_compile \
    ...
    "$MODEL_DIR/validate_fcstd_geometry.py" \
    "$MODEL_DIR/validate_step_roundtrip_direct.py" \
    "$MODEL_DIR/audit_forbidden_penetrations.py" \
    "$MODEL_DIR/audit_anchor_wind_geometry.py" \
    "$MODEL_DIR/assemble_validation_receipt.py" \
...
# 平面图 8/18 号与说明 13/27 号同时保留，不取平均，也不静默覆盖。
...
# Step 3: 原候选建模器仍读取少数旧字段名来写 sourceRef。这里只建立正确值的兼容别名，
...
# 初次阶段继续保留，用于展示候选外包络如何在后续专项步骤中被纠正。
...
# Step 9A: 新 FreeCADCmd 进程只打开原生 FCStd，检查对象、BRep、数量、质量体积、
# 吊杆合同、风缆弦长和主缆锚端横向散开；该进程不读取 STEP。
"$FREECAD_CMD" "$MODEL_DIR/validate_fcstd_geometry.py" \
  2>&1 | tee "$OUT_DIR/logs/09a-native-fcstd-validation.log"
assert_json_equals "$OUT_DIR/fcstd_validation_report.json" "status" "PASS"
assert_json_equals "$OUT_DIR/fcstd_validation_report.json" "engineeringReleaseStatus" "BLOCKED"

# Step 9B: 第二个独立 FreeCADCmd 进程只读取 STEP。它通过 Part.TopoShape.read
# 将 675 个 solids 读成一个 TopoShape，避免 OCAF/文档对象展开；然后比较实体数、
# 体积和包围盒，并保存单对象 round-trip FCStd。
"$FREECAD_CMD" "$MODEL_DIR/validate_step_roundtrip_direct.py" \
  2>&1 | tee "$OUT_DIR/logs/09b-step-roundtrip-validation.log"
assert_json_equals "$OUT_DIR/step_roundtrip_report.json" "status" "PASS"
...
# Step 11: 新进程专项验证锚碇台阶、横向槽、索面散开，以及四条风缆/锚座控制坐标。
...
# Step 12: 使用普通 CPython 汇总四份已经封存的独立报告；该脚本无 BRep 修改能力。
python3 "$MODEL_DIR/assemble_validation_receipt.py" \
  2>&1 | tee "$OUT_DIR/logs/12-validation-receipt-assembly.log"
assert_json_equals "$OUT_DIR/validation_report.json" "technicalStatus" "PASS"
assert_json_equals "$OUT_DIR/validation_report.json" "engineeringReleaseStatus" "BLOCKED"
assert_json_equals "$OUT_DIR/gate_receipt.json" "technicalGeometryValidation" "PASS"

# Step 13: 在 Xvfb 虚拟显示中保存四个最终视图，并逐图执行非白像素门禁。
QT_QPA_PLATFORM=xcb timeout 180s xvfb-run -a "$FREECAD_GUI" "$MODEL_DIR/render_freecad_model.py" \
  2>&1 | tee "$OUT_DIR/logs/13-freecad-render.log"
assert_json_equals "$OUT_DIR/renders/render_report.json" "status" "PASS"

# Step 14: 把本次实际使用的脚本和事实清单复制到交付物，便于逐行复核。
for file in \
  source_facts.json \
  ...
  validate_fcstd_geometry.py \
  validate_step_roundtrip_direct.py \
  audit_forbidden_penetrations.py \
  audit_anchor_wind_geometry.py \
  assemble_validation_receipt.py \
  render_freecad_model.py \
  run_freecad_pipeline.sh \
  ANCHOR_WIND_REVIEW.md \
  anchor_wind_review_facts.json \
  README.md; do
  cp "$MODEL_DIR/$file" "$OUT_DIR/process_sources/$file"
done

# Step 15: 文件级终检。工程 Gate 可以是 BLOCKED，但技术文件不得缺失或为空。
for required in \
  ...
  fcstd_validation_report.json \
  step_roundtrip_report.json \
...
# Step 16: 生成单一下载包，排除自身，保留目录结构、脚本、日志和全部阶段快照。
(
  cd "$OUT_DIR"
