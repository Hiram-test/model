...
# Compound STEP 封装、STEP 回读、禁止穿透审计或渲染失败，最终退出码均非零；
# 上层 Actions 会先上传已有日志和中间文件，再执行最终失败判定。
set -euo pipefail
...
# Step 4: 将 217 个 DISPLAY 对象封装为一个顶层 Compound STEP。
# FreeCAD 0.19 回读数百个顶层 free shapes 时可能段错误；对象身份由 FCStd 和 sidecar manifest 保留。
"$FREECAD_CMD" "$MODEL_DIR/export_compound_step.py" \
  2>&1 | tee "$OUT_DIR/logs/04-compound-step-export.log"

# Step 5: 使用独立脚本重新打开修正后的 FCStd 和 Compound STEP，不复用建模器内存状态。
"$FREECAD_CMD" "$MODEL_DIR/validate_freecad_model.py" \
  2>&1 | tee "$OUT_DIR/logs/05-independent-validation.log"

# Step 6: 独立执行 OpenCASCADE 公共体积审计；锚固端未闭合接口不伪装成已通过。
"$FREECAD_CMD" "$MODEL_DIR/audit_forbidden_penetrations.py" \
  2>&1 | tee "$OUT_DIR/logs/06-forbidden-penetration-audit.log"

# Step 7: 在 Xvfb 虚拟显示中使用 FreeCAD GUI 保存四个视图。
...
QT_QPA_PLATFORM=xcb timeout 180s xvfb-run -a "$FREECAD_GUI" "$MODEL_DIR/render_freecad_model.py" \
  2>&1 | tee "$OUT_DIR/logs/07-freecad-render.log"

# Step 8: 把本次实际使用的脚本和事实清单复制到交付物，便于逐行复核。
...
# Step 9: 文件级终检。工程 Gate 可以是 BLOCKED，但技术文件不得缺失或为空。
...
# Step 10: 生成单一下载包，排除自身，保留目录结构、脚本、日志和所有阶段快照。
(
  cd "$OUT_DIR"
